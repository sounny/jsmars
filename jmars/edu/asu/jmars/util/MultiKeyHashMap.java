...
/**
 * MultiKeyHashMap storage of values with two levels of key hierarchy,
 * super key (K1) and sub key (K2). The objects are inserted using super and sub keys.
 * It is not mandatory to use both keys as the user can use two keys to store
 * the values and use either of the keys to retrieve it.
 * 
 * @author Prathab K author of original version
 *
 */
public class MultiKeyHashMap<K1,K2,V> {
     ...
     * @param v
     * @return true if Object v is found
     */
    public boolean containsValue(V v) {
    	if (mkMap.containsValue(v)) {
    		return true;
    	} else {
    		return false;
    	}
