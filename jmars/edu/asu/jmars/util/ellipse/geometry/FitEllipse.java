 ...
 *  
 *  Sourced from: http://www.javased.com/index.php?source_dir=BoneJ/src/org/doube/geometry/FitEllipsoid.java
 */

import java.awt.geom.Point2D;
import java.util.List;

import edu.asu.jmars.util.ellipse.jama.EigenvalueDecomposition;
import edu.asu.jmars.util.ellipse.jama.Matrix;
...
public class FitEllipse {
	 ...
	 *            </p>
	 * @return <p>
	 *         array containing centroid coordinates, semi-axis lengths and angle of
	...
	/**
	 * Pass in an array of points (in world or screen, not spatial),
	 * and get back an Ellipse object containing the center point,
	 * a and b axes lengths (in degrees of the coords) and rotation
	 * angle (in radians).
	 * @param points Points defining the ellipse
	 * @return Ellipse fitting in those points
	 */
	public static Ellipse getEllipseFromPoints(List<Point2D> points){
		//create a double[][] out of the points
		double[][] pointsArr = new double[5][2];
		for(int i = 0; i<points.size(); i++){
			Point2D pt = points.get(i);
			pointsArr[i][0] = pt.getX();
			pointsArr[i][1] = pt.getY();
		}
		
		double[] dirResults = direct(pointsArr);
		
		return new Ellipse(varToDimensions(dirResults));
	}
