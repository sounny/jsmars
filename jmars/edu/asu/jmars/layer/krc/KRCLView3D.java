...
public class KRCLView3D extends LView3D {
	...
	public KRCLView3D(KRCLayer layer) {
		super(layer);
		myLayer = layer;
	 ...
	 * @param view
	 */
	public void setLView(KRCLView view){
		myLView = view;
	}
...
//		return true;
	 ...
	 * krc data point that has their label visible
	 */
	public ArrayList<GLRenderable> getRenderables(){
		
		final int layerState = myLayer.getStateId(KRCLayer.LABELS_BUFFER);
		...
		if(currentLabelStateId != layerState){
			...
			for(KRCDataPoint dp : myLayer.getKRCDataPoints()){
				if(layerState != myLayer.getStateId(KRCLayer.LABELS_BUFFER)){
	 ...
	 * 
	 * Populates the decals with the markers for each krc
	 * data point that is visible
	 */
	public DecalSet getDecals(){
		
		final int layerState = myLayer.getStateId(KRCLayer.IMAGES_BUFFER);
		...
		if(currentImageStateId != layerState){
			...
			if(!decals.isEmpty()){
				final Thread manager = new Thread(new Runnable() {
					public void run() {
						for(Decal d : decals){
							if(layerState != myLayer.getStateId(KRCLayer.IMAGES_BUFFER)){
