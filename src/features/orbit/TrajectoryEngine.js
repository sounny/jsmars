...
export class TrajectoryEngine {
  ...
  /**
   * Calculate interplanetary direct transfer trajectory from Mars to scattered disc dwarf planet candidate (471143) Dziewanna and orbit capture.
   * a = ( r_mars + r_dziewanna ) / 2
   * e = ( r_dziewanna - r_mars ) / ( r_dziewanna + r_mars )
   * Reference: Pál et al. (2012), Müller et al. (2018), Curtis (2013) for Scattered Disc Exploration.
   * @param {number} [marsParkingAltitudeKm=300.0] - Mars parking orbit altitude in km (150 to 1000 km)
   * @param {number} [dziewannaDistanceAU=38.50] - Dziewanna heliocentric distance in AU (30.0 to 55.0 AU)
   * @param {number} [dziewannaPeriapsisAltitudeKm=100.0] - Dziewanna orbit insertion periapsis altitude in km (30 to 5000 km)
   * @returns {{semiMajorAxisAU: number, eccentricity: number, timeOfFlightDays: number, timeOfFlightYears: number, marsDepartureDeltaVKmS: number, dziewannaOrbitInsertionDeltaVKmS: number, totalMissionDeltaVKmS: number, dziewannaContext: string}}
   */
  static computeMarsToDziewannaTransfer(marsParkingAltitudeKm = 300.0, dziewannaDistanceAU = 38.50, dziewannaPeriapsisAltitudeKm = 100.0) {
    const hpMarsKm = Math.max(150.0, marsParkingAltitudeKm);
    const rDAU = Math.max(30.0, Math.min(60.0, dziewannaDistanceAU));
    const hpDKm = Math.max(30.0, dziewannaPeriapsisAltitudeKm);

    const AU_KM = 1.495978707e8;
    const muSun = 1.32712440018e11;
    const muMars = 42828.37;
    const rMarsKm = 3389.5;
    const muDzie = 12.0; // km^3/s^2
    const rDzieKm = 250.0; // km
    const rMarsAU = 1.52368;

    const rMarsDistKm = rMarsAU * AU_KM;
    const rDDistKm = rDAU * AU_KM;

    const aKm = (rMarsDistKm + rDDistKm) / 2.0;
    const aAU = aKm / AU_KM;
    const ecc = (rDDistKm - rMarsDistKm) / (rDDistKm + rMarsDistKm);
    ...
    const vParkMarsKmS = Math.sqrt(muMars / rParkMarsKm);
    const vHypMarsKmS = Math.sqrt(Math.pow(vInfMarsKmS, 2.0) + ((2.0 * muMars) / rParkMarsKm));
    const dvTdiKmS = vHypMarsKmS - vParkMarsKmS;

    // Dziewanna capture
    const vDCircKmS = Math.sqrt(muSun / rDDistKm);
    const vArrKmS = Math.sqrt(muSun * ((2.0 / rDDistKm) - (1.0 / aKm)));
    const vInfDKmS = Math.abs(vDCircKmS - vArrKmS);

    const rpDKm = rDzieKm + hpDKm;
    const eCap = 0.85; // Capture orbit
    const aCapKm = rpDKm / (1.0 - eCap);

    const vHypDKmS = Math.sqrt(Math.pow(vInfDKmS, 2.0) + ((2.0 * muDzie) / rpDKm));
    const vCapDKmS = Math.sqrt(muDzie * ((2.0 / rpDKm) - (1.0 / aCapKm)));
    const dvDoiKmS = vHypDKmS - vCapDKmS;

    const dvTotKmS = dvTdiKmS + dvDoiKmS;
    ...
    return {
      ...
      timeOfFlightDays: parseFloat(tofDays.toFixed(1)),
      timeOfFlightYears: parseFloat(tofYrs.toFixed(2)),
      marsDepartureDeltaVKmS: parseFloat(dvTdiKmS.toFixed(3)),
      dziewannaOrbitInsertionDeltaVKmS: parseFloat(dvDoiKmS.toFixed(3)),
      totalMissionDeltaVKmS: parseFloat(dvTotKmS.toFixed(3)),
  ...
  /**
   * Calculate interplanetary direct transfer trajectory from Mars to binary Centaur / scattered disc object (42355) Typhon-Echidna and orbit capture.
   * a = ( r_mars + r_typhon ) / 2
   * e = ( r_typhon - r_mars ) / ( r_typhon + r_mars )
   * Reference: Grundy et al. (2008), Santos-Sanz et al. (2012), Curtis (2013) for Binary Centaur Exploration.
   * @param {number} [marsParkingAltitudeKm=300.0] - Mars parking orbit altitude in km (150 to 1000 km)
   * @param {number} [typhonDistanceAU=37.80] - Typhon heliocentric distance in AU (20.0 to 50.0 AU)
   * @param {number} [typhonPeriapsisAltitudeKm=40.0] - Typhon orbit insertion periapsis altitude in km (10 to 5000 km)
   * @returns {{semiMajorAxisAU: number, eccentricity: number, timeOfFlightDays: number, timeOfFlightYears: number, marsDepartureDeltaVKmS: number, typhonOrbitInsertionDeltaVKmS: number, totalMissionDeltaVKmS: number, typhonContext: string}}
   */
  static computeMarsToTyphonTransfer(marsParkingAltitudeKm = 300.0, typhonDistanceAU = 37.80, typhonPeriapsisAltitudeKm = 40.0) {
    const hpMarsKm = Math.max(150.0, marsParkingAltitudeKm);
    const rTAU = Math.max(15.0, Math.min(60.0, typhonDistanceAU));
    const hpTKm = Math.max(10.0, typhonPeriapsisAltitudeKm);

    const AU_KM = 1.495978707e8;
    const muSun = 1.32712440018e11;
    const muMars = 42828.37;
    const rMarsKm = 3389.5;
    const muTyph = 6.30; // km^3/s^2
    const rTyphKm = 80.0; // km
    const rMarsAU = 1.52368;

    const rMarsDistKm = rMarsAU * AU_KM;
    const rTDistKm = rTAU * AU_KM;

    const aKm = (rMarsDistKm + rTDistKm) / 2.0;
    const aAU = aKm / AU_KM;
    const ecc = (rTDistKm - rMarsDistKm) / (rTDistKm + rMarsDistKm);
    ...
    const vParkMarsKmS = Math.sqrt(muMars / rParkMarsKm);
    const vHypMarsKmS = Math.sqrt(Math.pow(vInfMarsKmS, 2.0) + ((2.0 * muMars) / rParkMarsKm));
    const dvTtiKmS = vHypMarsKmS - vParkMarsKmS;

    // Typhon capture
    const vTCircKmS = Math.sqrt(muSun / rTDistKm);
    const vArrKmS = Math.sqrt(muSun * ((2.0 / rTDistKm) - (1.0 / aKm)));
    const vInfTKmS = Math.abs(vTCircKmS - vArrKmS);

    const rpTKm = rTyphKm + hpTKm;
    const eCap = 0.85; // Capture orbit
    const aCapKm = rpTKm / (1.0 - eCap);

    const vHypTKmS = Math.sqrt(Math.pow(vInfTKmS, 2.0) + ((2.0 * muTyph) / rpTKm));
    const vCapTKmS = Math.sqrt(muTyph * ((2.0 / rpTKm) - (1.0 / aCapKm)));
    const dvToiKmS = vHypTKmS - vCapTKmS;

    const dvTotKmS = dvTtiKmS + dvToiKmS;
    ...
    return {
      ...
      timeOfFlightDays: parseFloat(tofDays.toFixed(1)),
      timeOfFlightYears: parseFloat(tofYrs.toFixed(2)),
      marsDepartureDeltaVKmS: parseFloat(dvTtiKmS.toFixed(3)),
      typhonOrbitInsertionDeltaVKmS: parseFloat(dvToiKmS.toFixed(3)),
      totalMissionDeltaVKmS: parseFloat(dvTotKmS.toFixed(3)),
  ...
  /**
   * Calculate interplanetary direct transfer trajectory from Mars to binary classical Kuiper Belt object (148780) Altjira and orbit capture.
   * a = ( r_mars + r_altjira ) / 2
   * e = ( r_altjira - r_mars ) / ( r_altjira + r_mars )
   * Reference: Grundy et al. (2011), Vilenius et al. (2014), Curtis (2013) for Classical Binary KBO Exploration.
   * @param {number} [marsParkingAltitudeKm=300.0] - Mars parking orbit altitude in km (150 to 1000 km)
   * @param {number} [altjiraDistanceAU=44.40] - Altjira heliocentric distance in AU (35.0 to 55.0 AU)
   * @param {number} [altjiraPeriapsisAltitudeKm=50.0] - Altjira orbit insertion periapsis altitude in km (20 to 5000 km)
   * @returns {{semiMajorAxisAU: number, eccentricity: number, timeOfFlightDays: number, timeOfFlightYears: number, marsDepartureDeltaVKmS: number, altjiraOrbitInsertionDeltaVKmS: number, totalMissionDeltaVKmS: number, altjiraContext: string}}
   */
  static computeMarsToAltjiraTransfer(marsParkingAltitudeKm = 300.0, altjiraDistanceAU = 44.40, altjiraPeriapsisAltitudeKm = 50.0) {
    const hpMarsKm = Math.max(150.0, marsParkingAltitudeKm);
    const rAAU = Math.max(30.0, Math.min(60.0, altjiraDistanceAU));
    const hpAKm = Math.max(20.0, altjiraPeriapsisAltitudeKm);

    const AU_KM = 1.495978707e8;
    const muSun = 1.32712440018e11;
    const muMars = 42828.37;
    const rMarsKm = 3389.5;
    const muAltj = 2.67; // km^3/s^2
    const rAltjKm = 100.0; // km
    const rMarsAU = 1.52368;
    ...
    const vACircKmS = Math.sqrt(muSun / rADistKm);
    const vArrKmS = Math.sqrt(muSun * ((2.0 / rADistKm) - (1.0 / aKm)));
    const vInfAKmS = Math.abs(vACircKmS - vArrKmS);

    const rpAKm = rAltjKm + hpAKm;
    const eCap = 0.85; // Capture orbit
    const aCapKm = rpAKm / (1.0 - eCap);

    const vHypAKmS = Math.sqrt(Math.pow(vInfAKmS, 2.0) + ((2.0 * muAltj) / rpAKm));
    const vCapAKmS = Math.sqrt(muAltj * ((2.0 / rpAKm) - (1.0 / aCapKm)));
    const dvAoiKmS = vHypAKmS - vCapAKmS;
    ...
    return {
      ...
      timeOfFlightYears: parseFloat(tofYrs.toFixed(2)),
      marsDepartureDeltaVKmS: parseFloat(dvTaiKmS.toFixed(3)),
      altjiraOrbitInsertionDeltaVKmS: parseFloat(dvAoiKmS.toFixed(3)),
      totalMissionDeltaVKmS: parseFloat(dvTotKmS.toFixed(3)),
  ...
  /**
   * Calculate interplanetary direct transfer trajectory from Mars to carbonaceous dwarf planet candidate (10) Hygiea and orbit capture.
   * a = ( r_mars + r_hygiea ) / 2
   * e = ( r_hygiea - r_mars ) / ( r_hygiea + r_mars )
   * Reference: Vernazza et al. (2019), Hanuš et al. (2020), Curtis (2013) for Outer Main Belt Exploration.
   * @param {number} [marsParkingAltitudeKm=300.0] - Mars parking orbit altitude in km (150 to 1000 km)
   * @param {number} [hygieaDistanceAU=3.140] - Hygiea heliocentric distance in AU (2.5 to 3.8 AU)
   * @param {number} [hygieaPeriapsisAltitudeKm=50.0] - Hygiea orbit insertion periapsis altitude in km (20 to 1000 km)
   * @returns {{semiMajorAxisAU: number, eccentricity: number, timeOfFlightDays: number, timeOfFlightYears: number, marsDepartureDeltaVKmS: number, hygieaOrbitInsertionDeltaVKmS: number, totalMissionDeltaVKmS: number, hygieaContext: string}}
   */
  static computeMarsToHygieaTransfer(marsParkingAltitudeKm = 300.0, hygieaDistanceAU = 3.140, hygieaPeriapsisAltitudeKm = 50.0) {
    const hpMarsKm = Math.max(150.0, marsParkingAltitudeKm);
    const rHAU = Math.max(2.0, Math.min(4.5, hygieaDistanceAU));
    const hpHKm = Math.max(10.0, hygieaPeriapsisAltitudeKm);

    const AU_KM = 1.495978707e8;
    const muSun = 1.32712440018e11;
    const muMars = 42828.37;
    const rMarsKm = 3389.5;
    const muHyg = 5.78; // km^3/s^2
    const rHygKm = 215.0; // km
    const rMarsAU = 1.52368;
    ...
    const rpHKm = rHygKm + hpHKm;
    const eCap = 0.85; // Capture orbit
    const aCapKm = rpHKm / (1.0 - eCap);

    const vHypHKmS = Math.sqrt(Math.pow(vInfHKmS, 2.0) + ((2.0 * muHyg) / rpHKm));
    const vCapHKmS = Math.sqrt(muHyg * ((2.0 / rpHKm) - (1.0 / aCapKm)));
    const dvHoiKmS = vHypHKmS - vCapHKmS;
    ...
    return {
      ...
      hygieaOrbitInsertionDeltaVKmS: parseFloat(dvHoiKmS.toFixed(3)),
      totalMissionDeltaVKmS: parseFloat(dvTotKmS.toFixed(3)),
      hygieaContext: `Mars-to-Hygiea (${tofYrs.toFixed(2)} yr TOF, e=${ecc.toFixed(4)}, Total Delta-V=${dvTotKmS.toFixed(2)} km/s, HOI=${dvHoiKmS.toFixed(2)} km/s)`
    };
  }

  /**
   * Calculate interplanetary direct transfer trajectory from Mars to Jupiter/Europa and Jovian moon orbit capture.
   * a = ( r_mars + r_jupiter ) / 2
   * e = ( r_jupiter - r_mars ) / ( r_jupiter + r_mars )
   * Reference: Europa Clipper Trajectory Baseline, Buffington et al. (2014), Curtis (2013) for Outer Planet Missions.
   * @param {number} [marsParkingAltitudeKm=300.0] - Mars parking orbit altitude in km (150 to 1000 km)
   * @param {number} [jupiterDistanceAU=5.2044] - Jupiter heliocentric distance in AU (4.9 to 5.5 AU)
   * @param {number} [europaOrbitAltitudeKm=100.0] - Europa science orbit altitude in km (50 to 500 km)
   * @returns {{semiMajorAxisAU: number, eccentricity: number, timeOfFlightDays: number, timeOfFlightYears: number, marsDepartureDeltaVKmS: number, europaOrbitInsertionDeltaVKmS: number, totalMissionDeltaVKmS: number, europaContext: string}}
   */
  static computeMarsToEuropaTransfer(marsParkingAltitudeKm = 300.0, jupiterDistanceAU = 5.2044, europaOrbitAltitudeKm = 100.0) {
    const hpMarsKm = Math.max(150.0, marsParkingAltitudeKm);
    const rJAU = Math.max(4.5, Math.min(6.0, jupiterDistanceAU));
    const hpEKm = Math.max(25.0, europaOrbitAltitudeKm);

    const AU_KM = 1.495978707e8;
    const muSun = 1.32712440018e11;
    const muMars = 42828.37;
    const rMarsKm = 3389.5;
    const muEuropa = 3202.73; // km^3/s^2
    const rEuropaKm = 1560.8; // km
    const rMarsAU = 1.52368;
    ...
    // Europa capture
    const vInfEKmS = 1.850; // km/s in Jovian resonant tour
    const rpEKm = rEuropaKm + hpEKm;
    const eCap = 0.50; // Science capture orbit
    const aCapKm = rpEKm / (1.0 - eCap);

    const vHypEKmS = Math.sqrt(Math.pow(vInfEKmS, 2.0) + ((2.0 * muEuropa) / rpEKm));
    const vCapEKmS = Math.sqrt(muEuropa * ((2.0 / rpEKm) - (1.0 / aCapKm)));
    const dvEoiKmS = vHypEKmS - vCapEKmS;

    const dvTotKmS = dvTjiMarsKmS + dvEoiKmS;
    ...
    return {
      ...
      europaOrbitInsertionDeltaVKmS: parseFloat(dvEoiKmS.toFixed(3)),
      totalMissionDeltaVKmS: parseFloat(dvTotKmS.toFixed(3)),
  ...
  /**
   * Calculate interplanetary direct 3D transfer trajectory from Mars to high-inclination asteroid (2) Pallas and orbit capture.
   * a = ( r_mars + r_pallas ) / 2
   * e = ( r_pallas - r_mars ) / ( r_pallas + r_mars )
   * Reference: Carry et al. (2012), Marsset et al. (2020), Curtis (2013) for High-Inclination Asteroid Missions.
   * @param {number} [marsParkingAltitudeKm=300.0] - Mars parking orbit altitude in km (150 to 1000 km)
   * @param {number} [pallasDistanceAU=2.772] - Pallas heliocentric distance in AU (2.1 to 3.4 AU)
   * @param {number} [pallasPeriapsisAltitudeKm=50.0] - Pallas orbit insertion periapsis altitude in km (20 to 1000 km)
   * @param {number} [inclinationPlaneChangeDeg=34.84] - Orbital plane change angle in degrees (0 to 45 deg)
   * @returns {{semiMajorAxisAU: number, eccentricity: number, timeOfFlightDays: number, timeOfFlightYears: number, marsDepartureDeltaVKmS: number, pallasOrbitInsertionDeltaVKmS: number, totalMissionDeltaVKmS: number, pallasContext: string}}
   */
  static computeMarsToPallasTransfer(marsParkingAltitudeKm = 300.0, pallasDistanceAU = 2.772, pallasPeriapsisAltitudeKm = 50.0, inclinationPlaneChangeDeg = 34.84) {
    const hpMarsKm = Math.max(150.0, marsParkingAltitudeKm);
    const rPAU = Math.max(1.8, Math.min(4.0, pallasDistanceAU));
    const hpPKm = Math.max(10.0, pallasPeriapsisAltitudeKm);
    const diDeg = Math.max(0.0, Math.min(60.0, inclinationPlaneChangeDeg));

    const AU_KM = 1.495978707e8;
    const muSun = 1.32712440018e11;
    const muMars = 42828.37;
    const rMarsKm = 3389.5;
    const muPallas = 13.70; // km^3/s^2
    const rPallasKm = 256.0; // km
    const rMarsAU = 1.52368;
    ...
    const rpPKm = rPallasKm + hpPKm;
    const eCap = 0.85; // Capture orbit
    const aCapKm = rpPKm / (1.0 - eCap);

    const vHypPKmS = Math.sqrt(Math.pow(vInfPKmS, 2.0) + ((2.0 * muPallas) / rpPKm));
    const vCapPKmS = Math.sqrt(muPallas * ((2.0 / rpPKm) - (1.0 / aCapKm)));
    const dvPoiKmS = vHypPKmS - vCapPKmS;
    ...
    return {
      ...
      pallasOrbitInsertionDeltaVKmS: parseFloat(dvPoiKmS.toFixed(3)),
      totalMissionDeltaVKmS: parseFloat(dvTotKmS.toFixed(3)),
      pallasContext: `Mars-to-Pallas (${tofYrs.toFixed(2)} yr TOF, inc=${diDeg.toFixed(1)} deg, Total Delta-V=${dvTotKmS.toFixed(2)} km/s, POI=${dvPoiKmS.toFixed(2)} km/s)`
    };
  }

  /**
   * Calculate interplanetary direct 3D transfer trajectory from Mars to stony main-belt asteroid (3) Juno and orbit capture.
   * a = ( r_mars + r_juno ) / 2
   * e = ( r_juno - r_mars ) / ( r_juno + r_mars )
   * Reference: Carry et al. (2012), Curtis (2013) for Main-Belt Asteroid Missions.
   * @param {number} [marsParkingAltitudeKm=300.0] - Mars parking orbit altitude in km (150 to 1000 km)
   * @param {number} [junoDistanceAU=2.670] - Juno heliocentric distance in AU (1.9 to 3.4 AU)
   * @param {number} [junoPeriapsisAltitudeKm=30.0] - Juno orbit insertion periapsis altitude in km (10 to 500 km)
   * @param {number} [inclinationPlaneChangeDeg=12.98] - Orbital plane change angle in degrees (0 to 30 deg)
   * @returns {{semiMajorAxisAU: number, eccentricity: number, timeOfFlightDays: number, timeOfFlightYears: number, marsDepartureDeltaVKmS: number, junoOrbitInsertionDeltaVKmS: number, totalMissionDeltaVKmS: number, junoContext: string}}
   */
  static computeMarsToJunoTransfer(marsParkingAltitudeKm = 300.0, junoDistanceAU = 2.670, junoPeriapsisAltitudeKm = 30.0, inclinationPlaneChangeDeg = 12.98) {
    const hpMarsKm = Math.max(150.0, marsParkingAltitudeKm);
    const rJAU = Math.max(1.8, Math.min(3.8, junoDistanceAU));
    const hpJKm = Math.max(5.0, junoPeriapsisAltitudeKm);
    ...
    const muMars = 42828.37;
    const rMarsKm = 3389.5;
    const muJuno = 19.10; // km^3/s^2
    const rJunoKm = 127.0; // km
    const rMarsAU = 1.52368;
    ...
    // Juno capture
    const vJCircKmS = Math.sqrt(muSun / rJDistKm);
    const vArrKmS = Math.sqrt(muSun * ((2.0 / rJDistKm) - (1.0 / aKm)));
    const vInfJKmS = Math.abs(vJCircKmS - vArrKmS);

    const rpJKm = rJunoKm + hpJKm;
    const eCap = 0.80; // Capture orbit
    const aCapKm = rpJKm / (1.0 - eCap);

    const vHypJKmS = Math.sqrt(Math.pow(vInfJKmS, 2.0) + ((2.0 * muJuno) / rpJKm));
    const vCapJKmS = Math.sqrt(muJuno * ((2.0 / rpJKm) - (1.0 / aCapKm)));
    const dvJoiKmS = vHypJKmS - vCapJKmS;

    const dvTotKmS = dvTjiMarsKmS + dvJoiKmS;
    ...
    return {
      ...
      timeOfFlightYears: parseFloat(tofYrs.toFixed(2)),
      marsDepartureDeltaVKmS: parseFloat(dvTjiMarsKmS.toFixed(3)),
      junoOrbitInsertionDeltaVKmS: parseFloat(dvJoiKmS.toFixed(3)),
      totalMissionDeltaVKmS: parseFloat(dvTotKmS.toFixed(3)),
      junoContext: `Mars-to-Juno (${tofYrs.toFixed(2)} yr TOF, inc=${diDeg.toFixed(1)} deg, Total Delta-V=${dvTotKmS.toFixed(2)} km/s, JOI=${dvJoiKmS.toFixed(2)} km/s)`
    };
  }

  /**
   * Calculate interplanetary direct 3D transfer trajectory from Mars to stony main-belt asteroid (5) Astraea and orbit capture.
   * a = ( r_mars + r_astraea ) / 2
   * e = ( r_astraea - r_mars ) / ( r_astraea + r_mars )
   * Reference: Carry et al. (2012), Curtis (2013) for Main-Belt Asteroid Missions.
   * @param {number} [marsParkingAltitudeKm=300.0] - Mars parking orbit altitude in km (150 to 1000 km)
   * @param {number} [astraeaDistanceAU=2.574] - Astraea heliocentric distance in AU (1.9 to 3.2 AU)
   * @param {number} [astraeaPeriapsisAltitudeKm=20.0] - Astraea orbit insertion periapsis altitude in km (5 to 300 km)
   * @param {number} [inclinationPlaneChangeDeg=5.37] - Orbital plane change angle in degrees (0 to 30 deg)
   * @returns {{semiMajorAxisAU: number, eccentricity: number, timeOfFlightDays: number, timeOfFlightYears: number, marsDepartureDeltaVKmS: number, astraeaOrbitInsertionDeltaVKmS: number, totalMissionDeltaVKmS: number, astraeaContext: string}}
   */
  static computeMarsToAstraeaTransfer(marsParkingAltitudeKm = 300.0, astraeaDistanceAU = 2.574, astraeaPeriapsisAltitudeKm = 20.0, inclinationPlaneChangeDeg = 5.37) {
    const hpMarsKm = Math.max(150.0, marsParkingAltitudeKm);
    const rAAU = Math.max(1.8, Math.min(3.5, astraeaDistanceAU));
    const hpAKm = Math.max(5.0, astraeaPeriapsisAltitudeKm);
    const diDeg = Math.max(0.0, Math.min(45.0, inclinationPlaneChangeDeg));
    ...
    const muMars = 42828.37;
    const rMarsKm = 3389.5;
    const muAstraea = 3.20; // km^3/s^2
    const rAstraeaKm = 59.5; // km
    const rMarsAU = 1.52368;
    ...
    const vParkMarsKmS = Math.sqrt(muMars / rParkMarsKm);
    const vHypMarsKmS = Math.sqrt(Math.pow(vInfMarsKmS, 2.0) + ((2.0 * muMars) / rParkMarsKm));
    const dvTaiMarsKmS = vHypMarsKmS - vParkMarsKmS;
    ...
    const rpAKm = rAstraeaKm + hpAKm;
    const eCap = 0.80; // Capture orbit
    const aCapKm = rpAKm / (1.0 - eCap);

    const vHypAKmS = Math.sqrt(Math.pow(vInfAKmS, 2.0) + ((2.0 * muAstraea) / rpAKm));
    const vCapAKmS = Math.sqrt(muAstraea * ((2.0 / rpAKm) - (1.0 / aCapKm)));
    const dvAoiKmS = vHypAKmS - vCapAKmS;

    const dvTotKmS = dvTaiMarsKmS + dvAoiKmS;
    ...
    return {
      ...
      timeOfFlightDays: parseFloat(tofDays.toFixed(1)),
      timeOfFlightYears: parseFloat(tofYrs.toFixed(2)),
      marsDepartureDeltaVKmS: parseFloat(dvTaiMarsKmS.toFixed(3)),
      astraeaOrbitInsertionDeltaVKmS: parseFloat(dvAoiKmS.toFixed(3)),
      totalMissionDeltaVKmS: parseFloat(dvTotKmS.toFixed(3)),
  ...
  /**
   * Calculate interplanetary direct 3D transfer trajectory from Mars to bright stony main-belt asteroid (7) Iris and orbit capture.
   * a = ( r_mars + r_iris ) / 2
   * e = ( r_iris - r_mars ) / ( r_iris + r_mars )
   * Reference: Carry et al. (2012), Marsset et al. (2020), Curtis (2013) for Main-Belt Asteroid Missions.
   * @param {number} [marsParkingAltitudeKm=300.0] - Mars parking orbit altitude in km (150 to 1000 km)
   * @param {number} [irisDistanceAU=2.386] - Iris heliocentric distance in AU (1.8 to 3.0 AU)
   * @param {number} [irisPeriapsisAltitudeKm=25.0] - Iris orbit insertion periapsis altitude in km (5 to 300 km)
   * @param {number} [inclinationPlaneChangeDeg=5.52] - Orbital plane change angle in degrees (0 to 30 deg)
   * @returns {{semiMajorAxisAU: number, eccentricity: number, timeOfFlightDays: number, timeOfFlightYears: number, marsDepartureDeltaVKmS: number, irisOrbitInsertionDeltaVKmS: number, totalMissionDeltaVKmS: number, irisContext: string}}
   */
  static computeMarsToIrisTransfer(marsParkingAltitudeKm = 300.0, irisDistanceAU = 2.386, irisPeriapsisAltitudeKm = 25.0, inclinationPlaneChangeDeg = 5.52) {
    const hpMarsKm = Math.max(150.0, marsParkingAltitudeKm);
    const rIAU = Math.max(1.7, Math.min(3.2, irisDistanceAU));
    const hpIKm = Math.max(5.0, irisPeriapsisAltitudeKm);
    const diDeg = Math.max(0.0, Math.min(45.0, inclinationPlaneChangeDeg));
    ...
    const muMars = 42828.37;
    const rMarsKm = 3389.5;
    const muIris = 8.70; // km^3/s^2
    const rIrisKm = 99.9; // km
    const rMarsAU = 1.52368;
    ...
    const vParkMarsKmS = Math.sqrt(muMars / rParkMarsKm);
    const vHypMarsKmS = Math.sqrt(Math.pow(vInfMarsKmS, 2.0) + ((2.0 * muMars) / rParkMarsKm));
    const dvTiiMarsKmS = vHypMarsKmS - vParkMarsKmS;
    ...
    const rpIKm = rIrisKm + hpIKm;
    const eCap = 0.80; // Capture orbit
    const aCapKm = rpIKm / (1.0 - eCap);

    const vHypIKmS = Math.sqrt(Math.pow(vInfIKmS, 2.0) + ((2.0 * muIris) / rpIKm));
    const vCapIKmS = Math.sqrt(muIris * ((2.0 / rpIKm) - (1.0 / aCapKm)));
    const dvIoiKmS = vHypIKmS - vCapIKmS;

    const dvTotKmS = dvTiiMarsKmS + dvIoiKmS;
    ...
    return {
      ...
      timeOfFlightDays: parseFloat(tofDays.toFixed(1)),
      timeOfFlightYears: parseFloat(tofYrs.toFixed(2)),
      marsDepartureDeltaVKmS: parseFloat(dvTiiMarsKmS.toFixed(3)),
      irisOrbitInsertionDeltaVKmS: parseFloat(dvIoiKmS.toFixed(3)),
      totalMissionDeltaVKmS: parseFloat(dvTotKmS.toFixed(3)),
  ...
  /**
   * Calculate interplanetary direct 3D transfer trajectory from Mars to large stony main-belt asteroid (9) Metis and orbit capture.
   * a = ( r_mars + r_metis ) / 2
   * e = ( r_metis - r_mars ) / ( r_metis + r_mars )
   * Reference: Carry et al. (2012), Marsset et al. (2020), Curtis (2013) for Main-Belt Asteroid Missions.
   * @param {number} [marsParkingAltitudeKm=300.0] - Mars parking orbit altitude in km (150 to 1000 km)
   * @param {number} [metisDistanceAU=2.387] - Metis heliocentric distance in AU (1.8 to 3.0 AU)
   * @param {number} [metisPeriapsisAltitudeKm=25.0] - Metis orbit insertion periapsis altitude in km (5 to 300 km)
   * @param {number} [inclinationPlaneChangeDeg=5.58] - Orbital plane change angle in degrees (0 to 30 deg)
   * @returns {{semiMajorAxisAU: number, eccentricity: number, timeOfFlightDays: number, timeOfFlightYears: number, marsDepartureDeltaVKmS: number, metisOrbitInsertionDeltaVKmS: number, totalMissionDeltaVKmS: number, metisContext: string}}
   */
  static computeMarsToMetisTransfer(marsParkingAltitudeKm = 300.0, metisDistanceAU = 2.387, metisPeriapsisAltitudeKm = 25.0, inclinationPlaneChangeDeg = 5.58) {
    const hpMarsKm = Math.max(150.0, marsParkingAltitudeKm);
    const rMAU = Math.max(1.7, Math.min(3.2, metisDistanceAU));
    const hpMKm = Math.max(5.0, metisPeriapsisAltitudeKm);
    const diDeg = Math.max(0.0, Math.min(45.0, inclinationPlaneChangeDeg));
    ...
    const muMars = 42828.37;
    const rMarsKm = 3389.5;
    const muMetis = 9.80; // km^3/s^2
    const rMetisKm = 95.0; // km
    const rMarsAU = 1.52368;
    ...
    const vParkMarsKmS = Math.sqrt(muMars / rParkMarsKm);
    const vHypMarsKmS = Math.sqrt(Math.pow(vInfMarsKmS, 2.0) + ((2.0 * muMars) / rParkMarsKm));
    const dvTmiMarsKmS = vHypMarsKmS - vParkMarsKmS;
    ...
    const rpMKm = rMetisKm + hpMKm;
    const eCap = 0.80; // Capture orbit
    const aCapKm = rpMKm / (1.0 - eCap);

    const vHypMKmS = Math.sqrt(Math.pow(vInfMKmS, 2.0) + ((2.0 * muMetis) / rpMKm));
    const vCapMKmS = Math.sqrt(muMetis * ((2.0 / rpMKm) - (1.0 / aCapKm)));
    const dvMoiKmS = vHypMKmS - vCapMKmS;

    const dvTotKmS = dvTmiMarsKmS + dvMoiKmS;
    ...
    return {
      ...
      timeOfFlightDays: parseFloat(tofDays.toFixed(1)),
      timeOfFlightYears: parseFloat(tofYrs.toFixed(2)),
      marsDepartureDeltaVKmS: parseFloat(dvTmiMarsKmS.toFixed(3)),
      metisOrbitInsertionDeltaVKmS: parseFloat(dvMoiKmS.toFixed(3)),
      totalMissionDeltaVKmS: parseFloat(dvTotKmS.toFixed(3)),
      metisContext: `Mars-to-Metis (${tofYrs.toFixed(2)} yr TOF, inc=${diDeg.toFixed(1)} deg, Total Delta-V=${dvTotKmS.toFixed(2)} km/s, MOI=${dvMoiKmS.toFixed(2)} km/s)`
    };
  }

  /**
   * Calculate interplanetary direct 3D transfer trajectory from Mars to large primitive C-type asteroid (10) Hygiea and orbit capture.
   * a = ( r_mars + r_hygiea ) / 2
   * e = ( r_hygiea - r_mars ) / ( r_hygiea + r_mars )
   * Reference: Vernazza et al. (2020), Carry et al. (2012), Curtis (2013) for Main-Belt Asteroid Missions.
   * @param {number} [marsParkingAltitudeKm=300.0] - Mars parking orbit altitude in km (150 to 1000 km)
   * @param {number} [hygieaDistanceAU=3.142] - Hygiea heliocentric distance in AU (2.5 to 3.6 AU)
   * @param {number} [hygieaPeriapsisAltitudeKm=50.0] - Hygiea orbit insertion periapsis altitude in km (10 to 500 km)
   * @param {number} [inclinationPlaneChangeDeg=3.84] - Orbital plane change angle in degrees (0 to 30 deg)
   * @returns {{semiMajorAxisAU: number, eccentricity: number, timeOfFlightDays: number, timeOfFlightYears: number, marsDepartureDeltaVKmS: number, hygieaOrbitInsertionDeltaVKmS: number, totalMissionDeltaVKmS: number, hygieaContext: string}}
   */
  static computeMarsToHygieaTransfer(marsParkingAltitudeKm = 300.0, hygieaDistanceAU = 3.142, hygieaPeriapsisAltitudeKm = 50.0, inclinationPlaneChangeDeg = 3.84) {
    const hpMarsKm = Math.max(150.0, marsParkingAltitudeKm);
    const rHAU = Math.max(2.4, Math.min(3.8, hygieaDistanceAU));
    const hpHKm = Math.max(10.0, hygieaPeriapsisAltitudeKm);
    const diDeg = Math.max(0.0, Math.min(45.0, inclinationPlaneChangeDeg));
    ...
    const muMars = 42828.37;
    const rMarsKm = 3389.5;
    const muHygiea = 5.78; // km^3/s^2
    const rHygieaKm = 217.0; // km
    const rMarsAU = 1.52368;
    ...
    const rpHKm = rHygieaKm + hpHKm;
    const eCap = 0.80; // Capture orbit
    const aCapKm = rpHKm / (1.0 - eCap);

    const vHypHKmS = Math.sqrt(Math.pow(vInfHKmS, 2.0) + ((2.0 * muHygiea) / rpHKm));
    const vCapHKmS = Math.sqrt(muHygiea * ((2.0 / rpHKm) - (1.0 / aCapKm)));
    const dvHoiKmS = vHypHKmS - vCapHKmS;
  ...
  /**
   * Calculate interplanetary direct 3D transfer trajectory from Mars to stony main-belt asteroid (12) Victoria and orbit capture.
   * a = ( r_mars + r_victoria ) / 2
   * e = ( r_victoria - r_mars ) / ( r_victoria + r_mars )
   * Reference: Carry et al. (2012), Marsset et al. (2020), Curtis (2013) for Main-Belt Asteroid Missions.
   * @param {number} [marsParkingAltitudeKm=300.0] - Mars parking orbit altitude in km (150 to 1000 km)
   * @param {number} [victoriaDistanceAU=2.334] - Victoria heliocentric distance in AU (1.8 to 2.9 AU)
   * @param {number} [victoriaPeriapsisAltitudeKm=15.0] - Victoria orbit insertion periapsis altitude in km (5 to 300 km)
   * @param {number} [inclinationPlaneChangeDeg=8.36] - Orbital plane change angle in degrees (0 to 30 deg)
   * @returns {{semiMajorAxisAU: number, eccentricity: number, timeOfFlightDays: number, timeOfFlightYears: number, marsDepartureDeltaVKmS: number, victoriaOrbitInsertionDeltaVKmS: number, totalMissionDeltaVKmS: number, victoriaContext: string}}
   */
  static computeMarsToVictoriaTransfer(marsParkingAltitudeKm = 300.0, victoriaDistanceAU = 2.334, victoriaPeriapsisAltitudeKm = 15.0, inclinationPlaneChangeDeg = 8.36) {
    const hpMarsKm = Math.max(150.0, marsParkingAltitudeKm);
    const rVAU = Math.max(1.7, Math.min(3.1, victoriaDistanceAU));
    const hpVKm = Math.max(5.0, victoriaPeriapsisAltitudeKm);
    const diDeg = Math.max(0.0, Math.min(45.0, inclinationPlaneChangeDeg));
    ...
    const muMars = 42828.37;
    const rMarsKm = 3389.5;
    const muVictoria = 3.75; // km^3/s^2
    const rVictoriaKm = 56.5; // km
    const rMarsAU = 1.52368;
    ...
    const vParkMarsKmS = Math.sqrt(muMars / rParkMarsKm);
    const vHypMarsKmS = Math.sqrt(Math.pow(vInfMarsKmS, 2.0) + ((2.0 * muMars) / rParkMarsKm));
    const dvTviMarsKmS = vHypMarsKmS - vParkMarsKmS;
    ...
    const rpVKm = rVictoriaKm + hpVKm;
    const eCap = 0.80; // Capture orbit
    const aCapKm = rpVKm / (1.0 - eCap);

    const vHypVKmS = Math.sqrt(Math.pow(vInfVKmS, 2.0) + ((2.0 * muVictoria) / rpVKm));
    const vCapVKmS = Math.sqrt(muVictoria * ((2.0 / rpVKm) - (1.0 / aCapKm)));
    const dvVoiKmS = vHypVKmS - vCapVKmS;

    const dvTotKmS = dvTviMarsKmS + dvVoiKmS;
    ...
    return {
      ...
      timeOfFlightDays: parseFloat(tofDays.toFixed(1)),
      timeOfFlightYears: parseFloat(tofYrs.toFixed(2)),
      marsDepartureDeltaVKmS: parseFloat(dvTviMarsKmS.toFixed(3)),
      victoriaOrbitInsertionDeltaVKmS: parseFloat(dvVoiKmS.toFixed(3)),
      totalMissionDeltaVKmS: parseFloat(dvTotKmS.toFixed(3)),
  ...
  /**
   * Calculate interplanetary direct 3D transfer trajectory from Mars to stony main-belt asteroid (17) Thetis and orbit capture.
   * a = ( r_mars + r_thetis ) / 2
   * e = ( r_thetis - r_mars ) / ( r_thetis + r_mars )
   * Reference: Carry et al. (2012), Marsset et al. (2020), Curtis (2013) for Main-Belt Asteroid Missions.
   * @param {number} [marsParkingAltitudeKm=300.0] - Mars parking orbit altitude in km (150 to 1000 km)
   * @param {number} [thetisDistanceAU=2.470] - Thetis heliocentric distance in AU (1.9 to 3.0 AU)
   * @param {number} [thetisPeriapsisAltitudeKm=15.0] - Thetis orbit insertion periapsis altitude in km (5 to 300 km)
   * @param {number} [inclinationPlaneChangeDeg=5.59] - Orbital plane change angle in degrees (0 to 30 deg)
   * @returns {{semiMajorAxisAU: number, eccentricity: number, timeOfFlightDays: number, timeOfFlightYears: number, marsDepartureDeltaVKmS: number, thetisOrbitInsertionDeltaVKmS: number, totalMissionDeltaVKmS: number, thetisContext: string}}
   */
  static computeMarsToThetisTransfer(marsParkingAltitudeKm = 300.0, thetisDistanceAU = 2.470, thetisPeriapsisAltitudeKm = 15.0, inclinationPlaneChangeDeg = 5.59) {
    const hpMarsKm = Math.max(150.0, marsParkingAltitudeKm);
    const rTAU = Math.max(1.8, Math.min(3.2, thetisDistanceAU));
    const hpTKm = Math.max(5.0, thetisPeriapsisAltitudeKm);
    const diDeg = Math.max(0.0, Math.min(45.0, inclinationPlaneChangeDeg));
    ...
    const muMars = 42828.37;
    const rMarsKm = 3389.5;
    const muThetis = 3.02; // km^3/s^2
    const rThetisKm = 45.0; // km
    const rMarsAU = 1.52368;
    ...
    const vParkMarsKmS = Math.sqrt(muMars / rParkMarsKm);
    const vHypMarsKmS = Math.sqrt(Math.pow(vInfMarsKmS, 2.0) + ((2.0 * muMars) / rParkMarsKm));
    const dvTtiMarsKmS = vHypMarsKmS - vParkMarsKmS;
    ...
    const rpTKm = rThetisKm + hpTKm;
    const eCap = 0.80; // Capture orbit
    const aCapKm = rpTKm / (1.0 - eCap);

    const vHypTKmS = Math.sqrt(Math.pow(vInfTKmS, 2.0) + ((2.0 * muThetis) / rpTKm));
    const vCapTKmS = Math.sqrt(muThetis * ((2.0 / rpTKm) - (1.0 / aCapKm)));
    const dvToiKmS = vHypTKmS - vCapTKmS;

    const dvTotKmS = dvTtiMarsKmS + dvToiKmS;
    ...
    return {
      ...
      timeOfFlightDays: parseFloat(tofDays.toFixed(1)),
      timeOfFlightYears: parseFloat(tofYrs.toFixed(2)),
      marsDepartureDeltaVKmS: parseFloat(dvTtiMarsKmS.toFixed(3)),
      thetisOrbitInsertionDeltaVKmS: parseFloat(dvToiKmS.toFixed(3)),
      totalMissionDeltaVKmS: parseFloat(dvTotKmS.toFixed(3)),
  ...
  /**
   * Calculate interplanetary direct 3D transfer trajectory from Mars to high-density stony S-type asteroid (20) Massalia and orbit capture.
   * a = ( r_mars + r_massalia ) / 2
   * e = ( r_massalia - r_mars ) / ( r_massalia + r_mars )
   * Reference: Carry et al. (2012), Bange (1998), Curtis (2013) for Main-Belt Asteroid Missions.
   * @param {number} [marsParkingAltitudeKm=300.0] - Mars parking orbit altitude in km (150 to 1000 km)
   * @param {number} [massaliaDistanceAU=2.409] - Massalia heliocentric distance in AU (1.8 to 3.0 AU)
   * @param {number} [massaliaPeriapsisAltitudeKm=20.0] - Massalia orbit insertion periapsis altitude in km (5 to 400 km)
   * @param {number} [inclinationPlaneChangeDeg=0.71] - Orbital plane change angle in degrees (0 to 25 deg)
   * @returns {{semiMajorAxisAU: number, eccentricity: number, timeOfFlightDays: number, timeOfFlightYears: number, marsDepartureDeltaVKmS: number, massaliaOrbitInsertionDeltaVKmS: number, totalMissionDeltaVKmS: number, massaliaContext: string}}
   */
  static computeMarsToMassaliaTransfer(marsParkingAltitudeKm = 300.0, massaliaDistanceAU = 2.409, massaliaPeriapsisAltitudeKm = 20.0, inclinationPlaneChangeDeg = 0.71) {
    const hpMarsKm = Math.max(150.0, marsParkingAltitudeKm);
    const rMaAU = Math.max(1.7, Math.min(3.1, massaliaDistanceAU));
    const hpMaKm = Math.max(5.0, massaliaPeriapsisAltitudeKm);
    const diDeg = Math.max(0.0, Math.min(45.0, inclinationPlaneChangeDeg));
    ...
    const muMars = 42828.37;
    const rMarsKm = 3389.5;
    const muMassalia = 3.47; // km^3/s^2
    const rMassaliaKm = 72.5; // km
    const rMarsAU = 1.52368;

    const rMarsDistKm = rMarsAU * AU_KM;
    const rMaDistKm = rMaAU * AU_KM;

    const aKm = (rMarsDistKm + rMaDistKm) / 2.0;
    const aAU = aKm / AU_KM;
    const ecc = (rMaDistKm - rMarsDistKm) / (rMaDistKm + rMarsDistKm);
    ...
    const vParkMarsKmS = Math.sqrt(muMars / rParkMarsKm);
    const vHypMarsKmS = Math.sqrt(Math.pow(vInfMarsKmS, 2.0) + ((2.0 * muMars) / rParkMarsKm));
    const dvTmaiMarsKmS = vHypMarsKmS - vParkMarsKmS;

    // Massalia capture
    const vMaCircKmS = Math.sqrt(muSun / rMaDistKm);
    const vArrKmS = Math.sqrt(muSun * ((2.0 / rMaDistKm) - (1.0 / aKm)));
    const vInfMaKmS = Math.abs(vMaCircKmS - vArrKmS);

    const rpMaKm = rMassaliaKm + hpMaKm;
    const eCap = 0.80; // Capture orbit
    const aCapKm = rpMaKm / (1.0 - eCap);

    const vHypMaKmS = Math.sqrt(Math.pow(vInfMaKmS, 2.0) + ((2.0 * muMassalia) / rpMaKm));
    const vCapMaKmS = Math.sqrt(muMassalia * ((2.0 / rpMaKm) - (1.0 / aCapKm)));
    const dvMaoiKmS = vHypMaKmS - vCapMaKmS;

    const dvTotKmS = dvTmaiMarsKmS + dvMaoiKmS;
    ...
    return {
      ...
      timeOfFlightDays: parseFloat(tofDays.toFixed(1)),
      timeOfFlightYears: parseFloat(tofYrs.toFixed(2)),
      marsDepartureDeltaVKmS: parseFloat(dvTmaiMarsKmS.toFixed(3)),
      massaliaOrbitInsertionDeltaVKmS: parseFloat(dvMaoiKmS.toFixed(3)),
      totalMissionDeltaVKmS: parseFloat(dvTotKmS.toFixed(3)),
      massaliaContext: `Mars-to-Massalia (${tofYrs.toFixed(2)} yr TOF, inc=${diDeg.toFixed(1)} deg, Total Delta-V=${dvTotKmS.toFixed(2)} km/s, MaOI=${dvMaoiKmS.toFixed(2)} km/s)`
    };
  }

  /**
   * Calculate interplanetary direct 3D transfer trajectory from Mars to dense primitive M-type asteroid (21) Lutetia and orbit capture.
   * a = ( r_mars + r_lutetia ) / 2
   * e = ( r_lutetia - r_mars ) / ( r_lutetia + r_mars )
   * Reference: Sierks et al. (2011), Pätzold et al. (2011), Curtis (2013) for Main-Belt Asteroid Missions.
   * @param {number} [marsParkingAltitudeKm=300.0] - Mars parking orbit altitude in km (150 to 1000 km)
   * @param {number} [lutetiaDistanceAU=2.435] - Lutetia heliocentric distance in AU (1.8 to 3.0 AU)
   * @param {number} [lutetiaPeriapsisAltitudeKm=15.0] - Lutetia orbit insertion periapsis altitude in km (5 to 350 km)
   * @param {number} [inclinationPlaneChangeDeg=3.06] - Orbital plane change angle in degrees (0 to 25 deg)
   * @returns {{semiMajorAxisAU: number, eccentricity: number, timeOfFlightDays: number, timeOfFlightYears: number, marsDepartureDeltaVKmS: number, lutetiaOrbitInsertionDeltaVKmS: number, totalMissionDeltaVKmS: number, lutetiaContext: string}}
   */
  static computeMarsToLutetiaTransfer(marsParkingAltitudeKm = 300.0, lutetiaDistanceAU = 2.435, lutetiaPeriapsisAltitudeKm = 15.0, inclinationPlaneChangeDeg = 3.06) {
    const hpMarsKm = Math.max(150.0, marsParkingAltitudeKm);
    const rLuAU = Math.max(1.7, Math.min(3.1, lutetiaDistanceAU));
    const hpLuKm = Math.max(5.0, lutetiaPeriapsisAltitudeKm);
    const diDeg = Math.max(0.0, Math.min(45.0, inclinationPlaneChangeDeg));
    ...
    const muMars = 42828.37;
    const rMarsKm = 3389.5;
    const muLutetia = 1.70; // km^3/s^2
    const rLutetiaKm = 49.0; // km
    const rMarsAU = 1.52368;

    const rMarsDistKm = rMarsAU * AU_KM;
    const rLuDistKm = rLuAU * AU_KM;

    const aKm = (rMarsDistKm + rLuDistKm) / 2.0;
    const aAU = aKm / AU_KM;
    const ecc = (rLuDistKm - rMarsDistKm) / (rLuDistKm + rMarsDistKm);
    ...
    const vParkMarsKmS = Math.sqrt(muMars / rParkMarsKm);
    const vHypMarsKmS = Math.sqrt(Math.pow(vInfMarsKmS, 2.0) + ((2.0 * muMars) / rParkMarsKm));
    const dvTluiMarsKmS = vHypMarsKmS - vParkMarsKmS;

    // Lutetia capture
    const vLuCircKmS = Math.sqrt(muSun / rLuDistKm);
    const vArrKmS = Math.sqrt(muSun * ((2.0 / rLuDistKm) - (1.0 / aKm)));
    const vInfLuKmS = Math.abs(vLuCircKmS - vArrKmS);

    const rpLuKm = rLutetiaKm + hpLuKm;
    const eCap = 0.80; // Capture orbit
    const aCapKm = rpLuKm / (1.0 - eCap);

    const vHypLuKmS = Math.sqrt(Math.pow(vInfLuKmS, 2.0) + ((2.0 * muLutetia) / rpLuKm));
    const vCapLuKmS = Math.sqrt(muLutetia * ((2.0 / rpLuKm) - (1.0 / aCapKm)));
    const dvLuoiKmS = vHypLuKmS - vCapLuKmS;

    const dvTotKmS = dvTluiMarsKmS + dvLuoiKmS;
    ...
    return {
      ...
      timeOfFlightDays: parseFloat(tofDays.toFixed(1)),
      timeOfFlightYears: parseFloat(tofYrs.toFixed(2)),
      marsDepartureDeltaVKmS: parseFloat(dvTluiMarsKmS.toFixed(3)),
      lutetiaOrbitInsertionDeltaVKmS: parseFloat(dvLuoiKmS.toFixed(3)),
      totalMissionDeltaVKmS: parseFloat(dvTotKmS.toFixed(3)),
  ...
  /**
   * Calculate interplanetary direct 3D transfer trajectory from Mars to high-inclination stony S-type asteroid (23) Thalia and orbit capture.
   * a = ( r_mars + r_thalia ) / 2
   * e = ( r_thalia - r_mars ) / ( r_thalia + r_mars )
   * Reference: Carry et al. (2012), Curtis (2013) for Main-Belt Asteroid Missions.
   * @param {number} [marsParkingAltitudeKm=300.0] - Mars parking orbit altitude in km (150 to 1000 km)
   * @param {number} [thaliaDistanceAU=2.628] - Thalia heliocentric distance in AU (1.9 to 3.3 AU)
   * @param {number} [thaliaPeriapsisAltitudeKm=15.0] - Thalia orbit insertion periapsis altitude in km (5 to 400 km)
   * @param {number} [inclinationPlaneChangeDeg=10.14] - Orbital plane change angle in degrees (0 to 35 deg)
   * @returns {{semiMajorAxisAU: number, eccentricity: number, timeOfFlightDays: number, timeOfFlightYears: number, marsDepartureDeltaVKmS: number, thaliaOrbitInsertionDeltaVKmS: number, totalMissionDeltaVKmS: number, thaliaContext: string}}
   */
  static computeMarsToThaliaTransfer(marsParkingAltitudeKm = 300.0, thaliaDistanceAU = 2.628, thaliaPeriapsisAltitudeKm = 15.0, inclinationPlaneChangeDeg = 10.14) {
    const hpMarsKm = Math.max(150.0, marsParkingAltitudeKm);
    const rThAU = Math.max(1.8, Math.min(3.4, thaliaDistanceAU));
    const hpThKm = Math.max(5.0, thaliaPeriapsisAltitudeKm);
    const diDeg = Math.max(0.0, Math.min(45.0, inclinationPlaneChangeDeg));
    ...
    const muMars = 42828.37;
    const rMarsKm = 3389.5;
    const muThalia = 3.15; // km^3/s^2
    const rThaliaKm = 53.5; // km
    const rMarsAU = 1.52368;

    const rMarsDistKm = rMarsAU * AU_KM;
    const rThDistKm = rThAU * AU_KM;

    const aKm = (rMarsDistKm + rThDistKm) / 2.0;
    const aAU = aKm / AU_KM;
    const ecc = (rThDistKm - rMarsDistKm) / (rThDistKm + rMarsDistKm);
    ...
    const vParkMarsKmS = Math.sqrt(muMars / rParkMarsKm);
    const vHypMarsKmS = Math.sqrt(Math.pow(vInfMarsKmS, 2.0) + ((2.0 * muMars) / rParkMarsKm));
    const dvTthiMarsKmS = vHypMarsKmS - vParkMarsKmS;

    // Thalia capture
    const vThCircKmS = Math.sqrt(muSun / rThDistKm);
    const vArrKmS = Math.sqrt(muSun * ((2.0 / rThDistKm) - (1.0 / aKm)));
    const vInfThKmS = Math.abs(vThCircKmS - vArrKmS);

    const rpThKm = rThaliaKm + hpThKm;
    const eCap = 0.80; // Capture orbit
    const aCapKm = rpThKm / (1.0 - eCap);

    const vHypThKmS = Math.sqrt(Math.pow(vInfThKmS, 2.0) + ((2.0 * muThalia) / rpThKm));
    const vCapThKmS = Math.sqrt(muThalia * ((2.0 / rpThKm) - (1.0 / aCapKm)));
    const dvThoiKmS = vHypThKmS - vCapThKmS;

    const dvTotKmS = dvTthiMarsKmS + dvThoiKmS;
    ...
    return {
      ...
      timeOfFlightDays: parseFloat(tofDays.toFixed(1)),
      timeOfFlightYears: parseFloat(tofYrs.toFixed(2)),
      marsDepartureDeltaVKmS: parseFloat(dvTthiMarsKmS.toFixed(3)),
      thaliaOrbitInsertionDeltaVKmS: parseFloat(dvThoiKmS.toFixed(3)),
      totalMissionDeltaVKmS: parseFloat(dvTotKmS.toFixed(3)),
  ...
  /**
   * Calculate interplanetary direct 3D transfer trajectory from Mars to main-belt stony S-type asteroid (28) Bellona and orbit capture.
   * a = ( r_mars + r_bellona ) / 2
   * e = ( r_bellona - r_mars ) / ( r_bellona + r_mars )
   * Reference: Carry et al. (2012), Curtis (2013) for Main-Belt Asteroid Missions.
   * @param {number} [marsParkingAltitudeKm=300.0] - Mars parking orbit altitude in km (150 to 1000 km)
   * @param {number} [bellonaDistanceAU=2.778] - Bellona heliocentric distance in AU (1.9 to 3.5 AU)
   * @param {number} [bellonaPeriapsisAltitudeKm=20.0] - Bellona orbit insertion periapsis altitude in km (5 to 350 km)
   * @param {number} [inclinationPlaneChangeDeg=9.40] - Orbital plane change angle in degrees (0 to 35 deg)
   * @returns {{semiMajorAxisAU: number, eccentricity: number, timeOfFlightDays: number, timeOfFlightYears: number, marsDepartureDeltaVKmS: number, bellonaOrbitInsertionDeltaVKmS: number, totalMissionDeltaVKmS: number, bellonaContext: string}}
   */
  static computeMarsToBellonaTransfer(marsParkingAltitudeKm = 300.0, bellonaDistanceAU = 2.778, bellonaPeriapsisAltitudeKm = 20.0, inclinationPlaneChangeDeg = 9.40) {
    const hpMarsKm = Math.max(150.0, marsParkingAltitudeKm);
    const rBeAU = Math.max(1.8, Math.min(3.6, bellonaDistanceAU));
    const hpBeKm = Math.max(5.0, bellonaPeriapsisAltitudeKm);
    const diDeg = Math.max(0.0, Math.min(45.0, inclinationPlaneChangeDeg));
    ...
    const muMars = 42828.37;
    const rMarsKm = 3389.5;
    const muBellona = 2.45; // km^3/s^2
    const rBellonaKm = 60.5; // km
    const rMarsAU = 1.52368;

    const rMarsDistKm = rMarsAU * AU_KM;
    const rBeDistKm = rBeAU * AU_KM;

    const aKm = (rMarsDistKm + rBeDistKm) / 2.0;
    const aAU = aKm / AU_KM;
    const ecc = (rBeDistKm - rMarsDistKm) / (rBeDistKm + rMarsDistKm);
    ...
    const vParkMarsKmS = Math.sqrt(muMars / rParkMarsKm);
    const vHypMarsKmS = Math.sqrt(Math.pow(vInfMarsKmS, 2.0) + ((2.0 * muMars) / rParkMarsKm));
    const dvTbeiMarsKmS = vHypMarsKmS - vParkMarsKmS;

    // Bellona capture
    const vBeCircKmS = Math.sqrt(muSun / rBeDistKm);
    const vArrKmS = Math.sqrt(muSun * ((2.0 / rBeDistKm) - (1.0 / aKm)));
    const vInfBeKmS = Math.abs(vBeCircKmS - vArrKmS);

    const rpBeKm = rBellonaKm + hpBeKm;
    const eCap = 0.80; // Capture orbit
    const aCapKm = rpBeKm / (1.0 - eCap);

    const vHypBeKmS = Math.sqrt(Math.pow(vInfBeKmS, 2.0) + ((2.0 * muBellona) / rpBeKm));
    const vCapBeKmS = Math.sqrt(muBellona * ((2.0 / rpBeKm) - (1.0 / aCapKm)));
    const dvBeoiKmS = vHypBeKmS - vCapBeKmS;

    const dvTotKmS = dvTbeiMarsKmS + dvBeoiKmS;
    ...
    return {
      ...
      timeOfFlightDays: parseFloat(tofDays.toFixed(1)),
      timeOfFlightYears: parseFloat(tofYrs.toFixed(2)),
      marsDepartureDeltaVKmS: parseFloat(dvTbeiMarsKmS.toFixed(3)),
      bellonaOrbitInsertionDeltaVKmS: parseFloat(dvBeoiKmS.toFixed(3)),
      totalMissionDeltaVKmS: parseFloat(dvTotKmS.toFixed(3)),
  ...
  /**
   * Calculate interplanetary direct 3D transfer trajectory from Mars to main-belt stony S-type asteroid (30) Urania and orbit capture.
   * a = ( r_mars + r_urania ) / 2
   * e = ( r_urania - r_mars ) / ( r_urania + r_mars )
   * Reference: Carry et al. (2012), Curtis (2013) for Main-Belt Asteroid Missions.
   * @param {number} [marsParkingAltitudeKm=300.0] - Mars parking orbit altitude in km (150 to 1000 km)
   * @param {number} [uraniaDistanceAU=2.366] - Urania heliocentric distance in AU (1.7 to 3.0 AU)
   * @param {number} [uraniaPeriapsisAltitudeKm=15.0] - Urania orbit insertion periapsis altitude in km (5 to 350 km)
   * @param {number} [inclinationPlaneChangeDeg=2.10] - Orbital plane change angle in degrees (0 to 30 deg)
   * @returns {{semiMajorAxisAU: number, eccentricity: number, timeOfFlightDays: number, timeOfFlightYears: number, marsDepartureDeltaVKmS: number, uraniaOrbitInsertionDeltaVKmS: number, totalMissionDeltaVKmS: number, uraniaContext: string}}
   */
  static computeMarsToUraniaTransfer(marsParkingAltitudeKm = 300.0, uraniaDistanceAU = 2.366, uraniaPeriapsisAltitudeKm = 15.0, inclinationPlaneChangeDeg = 2.10) {
    const hpMarsKm = Math.max(150.0, marsParkingAltitudeKm);
    const rUrAU = Math.max(1.6, Math.min(3.2, uraniaDistanceAU));
    const hpUrKm = Math.max(5.0, uraniaPeriapsisAltitudeKm);
    const diDeg = Math.max(0.0, Math.min(45.0, inclinationPlaneChangeDeg));
    ...
    const muMars = 42828.37;
    const rMarsKm = 3389.5;
    const muUrania = 2.15; // km^3/s^2
    const rUraniaKm = 50.0; // km
    const rMarsAU = 1.52368;

    const rMarsDistKm = rMarsAU * AU_KM;
    const rUrDistKm = rUrAU * AU_KM;

    const aKm = (rMarsDistKm + rUrDistKm) / 2.0;
    const aAU = aKm / AU_KM;
    const ecc = (rUrDistKm - rMarsDistKm) / (rUrDistKm + rMarsDistKm);
    ...
    const vParkMarsKmS = Math.sqrt(muMars / rParkMarsKm);
    const vHypMarsKmS = Math.sqrt(Math.pow(vInfMarsKmS, 2.0) + ((2.0 * muMars) / rParkMarsKm));
    const dvTuriMarsKmS = vHypMarsKmS - vParkMarsKmS;

    // Urania capture
    const vUrCircKmS = Math.sqrt(muSun / rUrDistKm);
    const vArrKmS = Math.sqrt(muSun * ((2.0 / rUrDistKm) - (1.0 / aKm)));
    const vInfUrKmS = Math.abs(vUrCircKmS - vArrKmS);

    const rpUrKm = rUraniaKm + hpUrKm;
    const eCap = 0.80; // Capture orbit
    const aCapKm = rpUrKm / (1.0 - eCap);

    const vHypUrKmS = Math.sqrt(Math.pow(vInfUrKmS, 2.0) + ((2.0 * muUrania) / rpUrKm));
    const vCapUrKmS = Math.sqrt(muUrania * ((2.0 / rpUrKm) - (1.0 / aCapKm)));
    const dvUroiKmS = vHypUrKmS - vCapUrKmS;

    const dvTotKmS = dvTuriMarsKmS + dvUroiKmS;
    ...
    return {
      ...
      timeOfFlightDays: parseFloat(tofDays.toFixed(1)),
      timeOfFlightYears: parseFloat(tofYrs.toFixed(2)),
      marsDepartureDeltaVKmS: parseFloat(dvTuriMarsKmS.toFixed(3)),
      uraniaOrbitInsertionDeltaVKmS: parseFloat(dvUroiKmS.toFixed(3)),
      totalMissionDeltaVKmS: parseFloat(dvTotKmS.toFixed(3)),
  ...
  /**
   * Calculate interplanetary direct 3D transfer trajectory from Mars to main-belt stony S-type asteroid (32) Pomona and orbit capture.
   * a = ( r_mars + r_pomona ) / 2
   * e = ( r_pomona - r_mars ) / ( r_pomona + r_mars )
   * Reference: Carry et al. (2012), Curtis (2013) for Main-Belt Asteroid Missions.
   * @param {number} [marsParkingAltitudeKm=300.0] - Mars parking orbit altitude in km (150 to 1000 km)
   * @param {number} [pomonaDistanceAU=2.587] - Pomona heliocentric distance in AU (1.8 to 3.4 AU)
   * @param {number} [pomonaPeriapsisAltitudeKm=15.0] - Pomona orbit insertion periapsis altitude in km (5 to 350 km)
   * @param {number} [inclinationPlaneChangeDeg=5.53] - Orbital plane change angle in degrees (0 to 35 deg)
   * @returns {{semiMajorAxisAU: number, eccentricity: number, timeOfFlightDays: number, timeOfFlightYears: number, marsDepartureDeltaVKmS: number, pomonaOrbitInsertionDeltaVKmS: number, totalMissionDeltaVKmS: number, pomonaContext: string}}
   */
  static computeMarsToPomonaTransfer(marsParkingAltitudeKm = 300.0, pomonaDistanceAU = 2.587, pomonaPeriapsisAltitudeKm = 15.0, inclinationPlaneChangeDeg = 5.53) {
    const hpMarsKm = Math.max(150.0, marsParkingAltitudeKm);
    const rPoAU = Math.max(1.7, Math.min(3.5, pomonaDistanceAU));
    const hpPoKm = Math.max(5.0, pomonaPeriapsisAltitudeKm);
    const diDeg = Math.max(0.0, Math.min(45.0, inclinationPlaneChangeDeg));
    ...
    const muMars = 42828.37;
    const rMarsKm = 3389.5;
    const muPomona = 1.85; // km^3/s^2 (stony asteroid, D~81 km)
    const rPomonaKm = 40.5; // km
    const rMarsAU = 1.52368;

    const rMarsDistKm = rMarsAU * AU_KM;
    const rPoDistKm = rPoAU * AU_KM;

    const aKm = (rMarsDistKm + rPoDistKm) / 2.0;
    const aAU = aKm / AU_KM;
    const ecc = (rPoDistKm - rMarsDistKm) / (rPoDistKm + rMarsDistKm);
    ...
    const vParkMarsKmS = Math.sqrt(muMars / rParkMarsKm);
    const vHypMarsKmS = Math.sqrt(Math.pow(vInfMarsKmS, 2.0) + ((2.0 * muMars) / rParkMarsKm));
    const dvTpoiMarsKmS = vHypMarsKmS - vParkMarsKmS;

    // Pomona capture
    const vPoCircKmS = Math.sqrt(muSun / rPoDistKm);
    const vArrKmS = Math.sqrt(muSun * ((2.0 / rPoDistKm) - (1.0 / aKm)));
    const vInfPoKmS = Math.abs(vPoCircKmS - vArrKmS);

    const rpPoKm = rPomonaKm + hpPoKm;
    const eCap = 0.80; // Capture orbit
    const aCapKm = rpPoKm / (1.0 - eCap);

    const vHypPoKmS = Math.sqrt(Math.pow(vInfPoKmS, 2.0) + ((2.0 * muPomona) / rpPoKm));
    const vCapPoKmS = Math.sqrt(muPomona * ((2.0 / rpPoKm) - (1.0 / aCapKm)));
    const dvPoiKmS = vHypPoKmS - vCapPoKmS;

    const dvTotKmS = dvTpoiMarsKmS + dvPoiKmS;
    ...
    return {
      ...
      timeOfFlightDays: parseFloat(tofDays.toFixed(1)),
      timeOfFlightYears: parseFloat(tofYrs.toFixed(2)),
      marsDepartureDeltaVKmS: parseFloat(dvTpoiMarsKmS.toFixed(3)),
      pomonaOrbitInsertionDeltaVKmS: parseFloat(dvPoiKmS.toFixed(3)),
      totalMissionDeltaVKmS: parseFloat(dvTotKmS.toFixed(3)),
  ...
  /**
   * Calculate interplanetary direct 3D transfer trajectory from Mars to main-belt carbonaceous C-type asteroid (34) Circe and orbit capture.
   * a = ( r_mars + r_circe ) / 2
   * e = ( r_circe - r_mars ) / ( r_circe + r_mars )
   * Reference: Carry et al. (2012), Curtis (2013) for Main-Belt Asteroid Missions.
   * @param {number} [marsParkingAltitudeKm=300.0] - Mars parking orbit altitude in km (150 to 1000 km)
   * @param {number} [circeDistanceAU=2.685] - Circe heliocentric distance in AU (1.8 to 3.5 AU)
   * @param {number} [circePeriapsisAltitudeKm=15.0] - Circe orbit insertion periapsis altitude in km (5 to 350 km)
   * @param {number} [inclinationPlaneChangeDeg=5.50] - Orbital plane change angle in degrees (0 to 35 deg)
   * @returns {{semiMajorAxisAU: number, eccentricity: number, timeOfFlightDays: number, timeOfFlightYears: number, marsDepartureDeltaVKmS: number, circeOrbitInsertionDeltaVKmS: number, totalMissionDeltaVKmS: number, circeContext: string}}
   */
  static computeMarsToCirceTransfer(marsParkingAltitudeKm = 300.0, circeDistanceAU = 2.685, circePeriapsisAltitudeKm = 15.0, inclinationPlaneChangeDeg = 5.50) {
    const hpMarsKm = Math.max(150.0, marsParkingAltitudeKm);
    const rCiAU = Math.max(1.7, Math.min(3.6, circeDistanceAU));
    const hpCiKm = Math.max(5.0, circePeriapsisAltitudeKm);
    const diDeg = Math.max(0.0, Math.min(45.0, inclinationPlaneChangeDeg));
    ...
    const muMars = 42828.37;
    const rMarsKm = 3389.5;
    const muCirce = 3.45; // km^3/s^2 (carbonaceous asteroid, D~113 km)
    const rCirceKm = 56.5; // km
    const rMarsAU = 1.52368;

    const rMarsDistKm = rMarsAU * AU_KM;
    const rCiDistKm = rCiAU * AU_KM;

    const aKm = (rMarsDistKm + rCiDistKm) / 2.0;
    const aAU = aKm / AU_KM;
    const ecc = (rCiDistKm - rMarsDistKm) / (rCiDistKm + rMarsDistKm);
    ...
    const vParkMarsKmS = Math.sqrt(muMars / rParkMarsKm);
    const vHypMarsKmS = Math.sqrt(Math.pow(vInfMarsKmS, 2.0) + ((2.0 * muMars) / rParkMarsKm));
    const dvTciiMarsKmS = vHypMarsKmS - vParkMarsKmS;

    // Circe capture
    const vCiCircKmS = Math.sqrt(muSun / rCiDistKm);
    const vArrKmS = Math.sqrt(muSun * ((2.0 / rCiDistKm) - (1.0 / aKm)));
    const vInfCiKmS = Math.abs(vCiCircKmS - vArrKmS);

    const rpCiKm = rCirceKm + hpCiKm;
    const eCap = 0.80; // Capture orbit
    const aCapKm = rpCiKm / (1.0 - eCap);

    const vHypCiKmS = Math.sqrt(Math.pow(vInfCiKmS, 2.0) + ((2.0 * muCirce) / rpCiKm));
    const vCapCiKmS = Math.sqrt(muCirce * ((2.0 / rpCiKm) - (1.0 / aCapKm)));
    const dvCioiKmS = vHypCiKmS - vCapCiKmS;

    const dvTotKmS = dvTciiMarsKmS + dvCioiKmS;
    ...
    return {
      ...
      timeOfFlightDays: parseFloat(tofDays.toFixed(1)),
      timeOfFlightYears: parseFloat(tofYrs.toFixed(2)),
      marsDepartureDeltaVKmS: parseFloat(dvTciiMarsKmS.toFixed(3)),
      circeOrbitInsertionDeltaVKmS: parseFloat(dvCioiKmS.toFixed(3)),
      totalMissionDeltaVKmS: parseFloat(dvTotKmS.toFixed(3)),
  ...
  /**
   * Calculate interplanetary direct 3D transfer trajectory from Mars to main-belt carbonaceous C-type asteroid (36) Atalante and orbit capture.
   * a = ( r_mars + r_atalante ) / 2
   * e = ( r_atalante - r_mars ) / ( r_atalante + r_mars )
   * Reference: Carry et al. (2012), Curtis (2013) for Main-Belt Asteroid Missions.
   * @param {number} [marsParkingAltitudeKm=300.0] - Mars parking orbit altitude in km (150 to 1000 km)
   * @param {number} [atalanteDistanceAU=2.747] - Atalante heliocentric distance in AU (1.8 to 3.6 AU)
   * @param {number} [atalantePeriapsisAltitudeKm=15.0] - Atalante orbit insertion periapsis altitude in km (5 to 350 km)
   * @param {number} [inclinationPlaneChangeDeg=18.43] - Orbital plane change angle in degrees (0 to 45 deg)
   * @returns {{semiMajorAxisAU: number, eccentricity: number, timeOfFlightDays: number, timeOfFlightYears: number, marsDepartureDeltaVKmS: number, atalanteOrbitInsertionDeltaVKmS: number, totalMissionDeltaVKmS: number, atalanteContext: string}}
   */
  static computeMarsToAtalanteTransfer(marsParkingAltitudeKm = 300.0, atalanteDistanceAU = 2.747, atalantePeriapsisAltitudeKm = 15.0, inclinationPlaneChangeDeg = 18.43) {
    const hpMarsKm = Math.max(150.0, marsParkingAltitudeKm);
    const rAtAU = Math.max(1.7, Math.min(3.7, atalanteDistanceAU));
    const hpAtKm = Math.max(5.0, atalantePeriapsisAltitudeKm);
    const diDeg = Math.max(0.0, Math.min(45.0, inclinationPlaneChangeDeg));
    ...
    const muMars = 42828.37;
    const rMarsKm = 3389.5;
    const muAtalante = 3.20; // km^3/s^2 (dark C-type asteroid, D~106 km)
    const rAtalanteKm = 53.0; // km
    const rMarsAU = 1.52368;

    const rMarsDistKm = rMarsAU * AU_KM;
    const rAtDistKm = rAtAU * AU_KM;

    const aKm = (rMarsDistKm + rAtDistKm) / 2.0;
    const aAU = aKm / AU_KM;
    const ecc = (rAtDistKm - rMarsDistKm) / (rAtDistKm + rMarsDistKm);
    ...
    const vParkMarsKmS = Math.sqrt(muMars / rParkMarsKm);
    const vHypMarsKmS = Math.sqrt(Math.pow(vInfMarsKmS, 2.0) + ((2.0 * muMars) / rParkMarsKm));
    const dvTatiMarsKmS = vHypMarsKmS - vParkMarsKmS;

    // Atalante capture
    const vAtCircKmS = Math.sqrt(muSun / rAtDistKm);
    const vArrKmS = Math.sqrt(muSun * ((2.0 / rAtDistKm) - (1.0 / aKm)));
    const vInfAtKmS = Math.abs(vAtCircKmS - vArrKmS);

    const rpAtKm = rAtalanteKm + hpAtKm;
    const eCap = 0.80; // Capture orbit
    const aCapKm = rpAtKm / (1.0 - eCap);

    const vHypAtKmS = Math.sqrt(Math.pow(vInfAtKmS, 2.0) + ((2.0 * muAtalante) / rpAtKm));
    const vCapAtKmS = Math.sqrt(muAtalante * ((2.0 / rpAtKm) - (1.0 / aCapKm)));
    const dvAtoiKmS = vHypAtKmS - vCapAtKmS;

    const dvTotKmS = dvTatiMarsKmS + dvAtoiKmS;
    ...
    return {
      ...
      timeOfFlightDays: parseFloat(tofDays.toFixed(1)),
      timeOfFlightYears: parseFloat(tofYrs.toFixed(2)),
      marsDepartureDeltaVKmS: parseFloat(dvTatiMarsKmS.toFixed(3)),
      atalanteOrbitInsertionDeltaVKmS: parseFloat(dvAtoiKmS.toFixed(3)),
      totalMissionDeltaVKmS: parseFloat(dvTotKmS.toFixed(3)),
      atalanteContext: `Mars-to-Atalante (${tofYrs.toFixed(2)} yr TOF, inc=${diDeg.toFixed(1)} deg, Total Delta-V=${dvTotKmS.toFixed(2)} km/s, AtOI=${dvAtoiKmS.toFixed(2)} km/s)`
    };
  }

  /**
   * Calculate interplanetary direct 3D transfer trajectory from Mars to main-belt stony S-type asteroid (37) Fides and orbit capture.
   * a = ( r_mars + r_fides ) / 2
   * e = ( r_fides - r_mars ) / ( r_fides + r_mars )
   * Reference: Carry et al. (2012), Curtis (2013) for Main-Belt Asteroid Missions.
   * @param {number} [marsParkingAltitudeKm=300.0] - Mars parking orbit altitude in km (150 to 1000 km)
   * @param {number} [fidesDistanceAU=2.642] - Fides heliocentric distance in AU (1.8 to 3.5 AU)
   * @param {number} [fidesPeriapsisAltitudeKm=15.0] - Fides orbit insertion periapsis altitude in km (5 to 350 km)
   * @param {number} [inclinationPlaneChangeDeg=3.07] - Orbital plane change angle in degrees (0 to 30 deg)
   * @returns {{semiMajorAxisAU: number, eccentricity: number, timeOfFlightDays: number, timeOfFlightYears: number, marsDepartureDeltaVKmS: number, fidesOrbitInsertionDeltaVKmS: number, totalMissionDeltaVKmS: number, fidesContext: string}}
   */
  static computeMarsToFidesTransfer(marsParkingAltitudeKm = 300.0, fidesDistanceAU = 2.642, fidesPeriapsisAltitudeKm = 15.0, inclinationPlaneChangeDeg = 3.07) {
    const hpMarsKm = Math.max(150.0, marsParkingAltitudeKm);
    const rFiAU = Math.max(1.7, Math.min(3.6, fidesDistanceAU));
    const hpFiKm = Math.max(5.0, fidesPeriapsisAltitudeKm);
    const diDeg = Math.max(0.0, Math.min(45.0, inclinationPlaneChangeDeg));
    ...
    const muMars = 42828.37;
    const rMarsKm = 3389.5;
    const muFides = 3.10; // km^3/s^2 (stony asteroid, D~108 km)
    const rFidesKm = 54.0; // km
    const rMarsAU = 1.52368;

    const rMarsDistKm = rMarsAU * AU_KM;
    const rFiDistKm = rFiAU * AU_KM;

    const aKm = (rMarsDistKm + rFiDistKm) / 2.0;
    const aAU = aKm / AU_KM;
    const ecc = (rFiDistKm - rMarsDistKm) / (rFiDistKm + rMarsDistKm);
    ...
    const vParkMarsKmS = Math.sqrt(muMars / rParkMarsKm);
    const vHypMarsKmS = Math.sqrt(Math.pow(vInfMarsKmS, 2.0) + ((2.0 * muMars) / rParkMarsKm));
    const dvTfiiMarsKmS = vHypMarsKmS - vParkMarsKmS;

    // Fides capture
    const vFiCircKmS = Math.sqrt(muSun / rFiDistKm);
    const vArrKmS = Math.sqrt(muSun * ((2.0 / rFiDistKm) - (1.0 / aKm)));
    const vInfFiKmS = Math.abs(vFiCircKmS - vArrKmS);

    const rpFiKm = rFidesKm + hpFiKm;
    const eCap = 0.80; // Capture orbit
    const aCapKm = rpFiKm / (1.0 - eCap);

    const vHypFiKmS = Math.sqrt(Math.pow(vInfFiKmS, 2.0) + ((2.0 * muFides) / rpFiKm));
    const vCapFiKmS = Math.sqrt(muFides * ((2.0 / rpFiKm) - (1.0 / aCapKm)));
    const dvFioiKmS = vHypFiKmS - vCapFiKmS;

    const dvTotKmS = dvTfiiMarsKmS + dvFioiKmS;
    ...
    return {
      ...
      timeOfFlightDays: parseFloat(tofDays.toFixed(1)),
      timeOfFlightYears: parseFloat(tofYrs.toFixed(2)),
      marsDepartureDeltaVKmS: parseFloat(dvTfiiMarsKmS.toFixed(3)),
      fidesOrbitInsertionDeltaVKmS: parseFloat(dvFioiKmS.toFixed(3)),
      totalMissionDeltaVKmS: parseFloat(dvTotKmS.toFixed(3)),
  ...
  /**
   * Calculate interplanetary direct 3D transfer trajectory from Mars to main-belt bright stony S-type asteroid (39) Laetitia and orbit capture.
   * a = ( r_mars + r_laetitia ) / 2
   * e = ( r_laetitia - r_mars ) / ( r_laetitia + r_mars )
   * Reference: Carry et al. (2012), Curtis (2013) for Main-Belt Asteroid Missions.
   * @param {number} [marsParkingAltitudeKm=300.0] - Mars parking orbit altitude in km (150 to 1000 km)
   * @param {number} [laetitiaDistanceAU=2.769] - Laetitia heliocentric distance in AU (1.8 to 3.7 AU)
   * @param {number} [laetitiaPeriapsisAltitudeKm=15.0] - Laetitia orbit insertion periapsis altitude in km (5 to 350 km)
   * @param {number} [inclinationPlaneChangeDeg=10.38] - Orbital plane change angle in degrees (0 to 35 deg)
   * @returns {{semiMajorAxisAU: number, eccentricity: number, timeOfFlightDays: number, timeOfFlightYears: number, marsDepartureDeltaVKmS: number, laetitiaOrbitInsertionDeltaVKmS: number, totalMissionDeltaVKmS: number, laetitiaContext: string}}
   */
  static computeMarsToLaetitiaTransfer(marsParkingAltitudeKm = 300.0, laetitiaDistanceAU = 2.769, laetitiaPeriapsisAltitudeKm = 15.0, inclinationPlaneChangeDeg = 10.38) {
    const hpMarsKm = Math.max(150.0, marsParkingAltitudeKm);
    const rLaAU = Math.max(1.7, Math.min(3.8, laetitiaDistanceAU));
    const hpLaKm = Math.max(5.0, laetitiaPeriapsisAltitudeKm);
    const diDeg = Math.max(0.0, Math.min(45.0, inclinationPlaneChangeDeg));
    ...
    const muMars = 42828.37;
    const rMarsKm = 3389.5;
    const muLaetitia = 6.80; // km^3/s^2 (bright S-type asteroid, D~149.5 km)
    const rLaetitiaKm = 75.0; // km
    const rMarsAU = 1.52368;

    const rMarsDistKm = rMarsAU * AU_KM;
    const rLaDistKm = rLaAU * AU_KM;

    const aKm = (rMarsDistKm + rLaDistKm) / 2.0;
    const aAU = aKm / AU_KM;
    const ecc = (rLaDistKm - rMarsDistKm) / (rLaDistKm + rMarsDistKm);
    ...
    const vParkMarsKmS = Math.sqrt(muMars / rParkMarsKm);
    const vHypMarsKmS = Math.sqrt(Math.pow(vInfMarsKmS, 2.0) + ((2.0 * muMars) / rParkMarsKm));
    const dvTlaiMarsKmS = vHypMarsKmS - vParkMarsKmS;

    // Laetitia capture
    const vLaCircKmS = Math.sqrt(muSun / rLaDistKm);
    const vArrKmS = Math.sqrt(muSun * ((2.0 / rLaDistKm) - (1.0 / aKm)));
    const vInfLaKmS = Math.abs(vLaCircKmS - vArrKmS);

    const rpLaKm = rLaetitiaKm + hpLaKm;
    const eCap = 0.80; // Capture orbit
    const aCapKm = rpLaKm / (1.0 - eCap);

    const vHypLaKmS = Math.sqrt(Math.pow(vInfLaKmS, 2.0) + ((2.0 * muLaetitia) / rpLaKm));
    const vCapLaKmS = Math.sqrt(muLaetitia * ((2.0 / rpLaKm) - (1.0 / aCapKm)));
    const dvLaoiKmS = vHypLaKmS - vCapLaKmS;

    const dvTotKmS = dvTlaiMarsKmS + dvLaoiKmS;
    ...
    return {
      ...
      timeOfFlightDays: parseFloat(tofDays.toFixed(1)),
      timeOfFlightYears: parseFloat(tofYrs.toFixed(2)),
      marsDepartureDeltaVKmS: parseFloat(dvTlaiMarsKmS.toFixed(3)),
      laetitiaOrbitInsertionDeltaVKmS: parseFloat(dvLaoiKmS.toFixed(3)),
      totalMissionDeltaVKmS: parseFloat(dvTotKmS.toFixed(3)),
  ...
  /**
   * Calculate interplanetary direct 3D transfer trajectory from Mars to main-belt stony S-type asteroid (42) Isis and orbit capture.
   * a = ( r_mars + r_isis ) / 2
   * e = ( r_isis - r_mars ) / ( r_isis + r_mars )
   * Reference: Carry et al. (2012), Curtis (2013) for Main-Belt Asteroid Missions.
   * @param {number} [marsParkingAltitudeKm=300.0] - Mars parking orbit altitude in km (150 to 1000 km)
   * @param {number} [isisDistanceAU=2.442] - Isis heliocentric distance in AU (1.7 to 3.4 AU)
   * @param {number} [isisPeriapsisAltitudeKm=15.0] - Isis orbit insertion periapsis altitude in km (5 to 350 km)
   * @param {number} [inclinationPlaneChangeDeg=8.53] - Orbital plane change angle in degrees (0 to 35 deg)
   * @returns {{semiMajorAxisAU: number, eccentricity: number, timeOfFlightDays: number, timeOfFlightYears: number, marsDepartureDeltaVKmS: number, isisOrbitInsertionDeltaVKmS: number, totalMissionDeltaVKmS: number, isisContext: string}}
   */
  static computeMarsToIsisTransfer(marsParkingAltitudeKm = 300.0, isisDistanceAU = 2.442, isisPeriapsisAltitudeKm = 15.0, inclinationPlaneChangeDeg = 8.53) {
    const hpMarsKm = Math.max(150.0, marsParkingAltitudeKm);
    const rIsAU = Math.max(1.6, Math.min(3.5, isisDistanceAU));
    const hpIsKm = Math.max(5.0, isisPeriapsisAltitudeKm);
    const diDeg = Math.max(0.0, Math.min(45.0, inclinationPlaneChangeDeg));
    ...
    const muMars = 42828.37;
    const rMarsKm = 3389.5;
    const muIsis = 2.80; // km^3/s^2 (stony asteroid, D~100.2 km)
    const rIsisKm = 50.1; // km
    const rMarsAU = 1.52368;

    const rMarsDistKm = rMarsAU * AU_KM;
    const rIsDistKm = rIsAU * AU_KM;

    const aKm = (rMarsDistKm + rIsDistKm) / 2.0;
    const aAU = aKm / AU_KM;
    const ecc = (rIsDistKm - rMarsDistKm) / (rIsDistKm + rMarsDistKm);
    ...
    const vParkMarsKmS = Math.sqrt(muMars / rParkMarsKm);
    const vHypMarsKmS = Math.sqrt(Math.pow(vInfMarsKmS, 2.0) + ((2.0 * muMars) / rParkMarsKm));
    const dvTsiiMarsKmS = vHypMarsKmS - vParkMarsKmS;

    // Isis capture
    const vIsCircKmS = Math.sqrt(muSun / rIsDistKm);
    const vArrKmS = Math.sqrt(muSun * ((2.0 / rIsDistKm) - (1.0 / aKm)));
    const vInfIsKmS = Math.abs(vIsCircKmS - vArrKmS);

    const rpIsKm = rIsisKm + hpIsKm;
    const eCap = 0.80; // Capture orbit
    const aCapKm = rpIsKm / (1.0 - eCap);

    const vHypIsKmS = Math.sqrt(Math.pow(vInfIsKmS, 2.0) + ((2.0 * muIsis) / rpIsKm));
    const vCapIsKmS = Math.sqrt(muIsis * ((2.0 / rpIsKm) - (1.0 / aCapKm)));
    const dvIsoiKmS = vHypIsKmS - vCapIsKmS;

    const dvTotKmS = dvTsiiMarsKmS + dvIsoiKmS;
    ...
    return {
      ...
      timeOfFlightDays: parseFloat(tofDays.toFixed(1)),
      timeOfFlightYears: parseFloat(tofYrs.toFixed(2)),
      marsDepartureDeltaVKmS: parseFloat(dvTsiiMarsKmS.toFixed(3)),
      isisOrbitInsertionDeltaVKmS: parseFloat(dvIsoiKmS.toFixed(3)),
      totalMissionDeltaVKmS: parseFloat(dvTotKmS.toFixed(3)),
      isisContext: `Mars-to-Isis (${tofYrs.toFixed(2)} yr TOF, inc=${diDeg.toFixed(1)} deg, Total Delta-V=${dvTotKmS.toFixed(2)} km/s, IsOI=${dvIsoiKmS.toFixed(2)} km/s)`
    };
  }

  /**
   * Calculate interplanetary direct 3D transfer trajectory from Mars to inner main-belt Flora-family stony S-type asteroid (43) Ariadne and orbit capture.
   * a = ( r_mars + r_ariadne ) / 2
   * e = ( r_ariadne - r_mars ) / ( r_ariadne + r_mars )
   * Reference: Carry et al. (2012), Curtis (2013) for Main-Belt Asteroid Missions.
   * @param {number} [marsParkingAltitudeKm=300.0] - Mars parking orbit altitude in km (150 to 1000 km)
   * @param {number} [ariadneDistanceAU=2.204] - Ariadne heliocentric distance in AU (1.6 to 3.1 AU)
   * @param {number} [ariadnePeriapsisAltitudeKm=15.0] - Ariadne orbit insertion periapsis altitude in km (5 to 350 km)
   * @param {number} [inclinationPlaneChangeDeg=3.47] - Orbital plane change angle in degrees (0 to 30 deg)
   * @returns {{semiMajorAxisAU: number, eccentricity: number, timeOfFlightDays: number, timeOfFlightYears: number, marsDepartureDeltaVKmS: number, ariadneOrbitInsertionDeltaVKmS: number, totalMissionDeltaVKmS: number, ariadneContext: string}}
   */
  static computeMarsToAriadneTransfer(marsParkingAltitudeKm = 300.0, ariadneDistanceAU = 2.204, ariadnePeriapsisAltitudeKm = 15.0, inclinationPlaneChangeDeg = 3.47) {
    const hpMarsKm = Math.max(150.0, marsParkingAltitudeKm);
    const rArAU = Math.max(1.55, Math.min(3.3, ariadneDistanceAU));
    const hpArKm = Math.max(5.0, ariadnePeriapsisAltitudeKm);
    const diDeg = Math.max(0.0, Math.min(45.0, inclinationPlaneChangeDeg));
    ...
    const muMars = 42828.37;
    const rMarsKm = 3389.5;
    const muAriadne = 1.20; // km^3/s^2 (Flora-family stony asteroid, D~71.0 km)
    const rAriadneKm = 35.5; // km
    const rMarsAU = 1.52368;

    const rMarsDistKm = rMarsAU * AU_KM;
    const rArDistKm = rArAU * AU_KM;

    const aKm = (rMarsDistKm + rArDistKm) / 2.0;
    const aAU = aKm / AU_KM;
    const ecc = (rArDistKm - rMarsDistKm) / (rArDistKm + rMarsDistKm);
    ...
    const vParkMarsKmS = Math.sqrt(muMars / rParkMarsKm);
    const vHypMarsKmS = Math.sqrt(Math.pow(vInfMarsKmS, 2.0) + ((2.0 * muMars) / rParkMarsKm));
    const dvTariMarsKmS = vHypMarsKmS - vParkMarsKmS;

    // Ariadne capture
    const vArCircKmS = Math.sqrt(muSun / rArDistKm);
    const vArrKmS = Math.sqrt(muSun * ((2.0 / rArDistKm) - (1.0 / aKm)));
    const vInfArKmS = Math.abs(vArCircKmS - vArrKmS);

    const rpArKm = rAriadneKm + hpArKm;
    const eCap = 0.80; // Capture orbit
    const aCapKm = rpArKm / (1.0 - eCap);

    const vHypArKmS = Math.sqrt(Math.pow(vInfArKmS, 2.0) + ((2.0 * muAriadne) / rpArKm));
    const vCapArKmS = Math.sqrt(muAriadne * ((2.0 / rpArKm) - (1.0 / aCapKm)));
    const dvAroiKmS = vHypArKmS - vCapArKmS;

    const dvTotKmS = dvTariMarsKmS + dvAroiKmS;
    ...
    return {
      ...
      timeOfFlightDays: parseFloat(tofDays.toFixed(1)),
      timeOfFlightYears: parseFloat(tofYrs.toFixed(2)),
      marsDepartureDeltaVKmS: parseFloat(dvTariMarsKmS.toFixed(3)),
      ariadneOrbitInsertionDeltaVKmS: parseFloat(dvAroiKmS.toFixed(3)),
      totalMissionDeltaVKmS: parseFloat(dvTotKmS.toFixed(3)),
      ariadneContext: `Mars-to-Ariadne (${tofYrs.toFixed(2)} yr TOF, inc=${diDeg.toFixed(1)} deg, Total Delta-V=${dvTotKmS.toFixed(2)} km/s, ArOI=${dvAroiKmS.toFixed(2)} km/s)`
    };
  }

  /**
   * Calculate interplanetary direct 3D transfer trajectory from Mars to inner main-belt enstatite-rich, high-albedo E-type asteroid (44) Nysa and orbit capture.
   * a = ( r_mars + r_nysa ) / 2
   * e = ( r_nysa - r_mars ) / ( r_nysa + r_mars )
   * Reference: Carry et al. (2012), Curtis (2013) for Main-Belt Asteroid Missions.
   * @param {number} [marsParkingAltitudeKm=300.0] - Mars parking orbit altitude in km (150 to 1000 km)
   * @param {number} [nysaDistanceAU=2.423] - Nysa heliocentric distance in AU (1.7 to 3.4 AU)
   * @param {number} [nysaPeriapsisAltitudeKm=15.0] - Nysa orbit insertion periapsis altitude in km (5 to 350 km)
   * @param {number} [inclinationPlaneChangeDeg=3.70] - Orbital plane change angle in degrees (0 to 30 deg)
   * @returns {{semiMajorAxisAU: number, eccentricity: number, timeOfFlightDays: number, timeOfFlightYears: number, marsDepartureDeltaVKmS: number, nysaOrbitInsertionDeltaVKmS: number, totalMissionDeltaVKmS: number, nysaContext: string}}
   */
  static computeMarsToNysaTransfer(marsParkingAltitudeKm = 300.0, nysaDistanceAU = 2.423, nysaPeriapsisAltitudeKm = 15.0, inclinationPlaneChangeDeg = 3.70) {
    const hpMarsKm = Math.max(150.0, marsParkingAltitudeKm);
    const rNyAU = Math.max(1.6, Math.min(3.5, nysaDistanceAU));
    const hpNyKm = Math.max(5.0, nysaPeriapsisAltitudeKm);
    const diDeg = Math.max(0.0, Math.min(45.0, inclinationPlaneChangeDeg));
    ...
    const muMars = 42828.37;
    const rMarsKm = 3389.5;
    const muNysa = 1.25; // km^3/s^2 (high-albedo E-type asteroid, D~70.6 km)
    const rNysaKm = 35.3; // km
    const rMarsAU = 1.52368;

    const rMarsDistKm = rMarsAU * AU_KM;
    const rNyDistKm = rNyAU * AU_KM;

    const aKm = (rMarsDistKm + rNyDistKm) / 2.0;
    const aAU = aKm / AU_KM;
    const ecc = (rNyDistKm - rMarsDistKm) / (rNyDistKm + rMarsDistKm);
    ...
    const vParkMarsKmS = Math.sqrt(muMars / rParkMarsKm);
    const vHypMarsKmS = Math.sqrt(Math.pow(vInfMarsKmS, 2.0) + ((2.0 * muMars) / rParkMarsKm));
    const dvTnyiMarsKmS = vHypMarsKmS - vParkMarsKmS;

    // Nysa capture
    const vNyCircKmS = Math.sqrt(muSun / rNyDistKm);
    const vArrKmS = Math.sqrt(muSun * ((2.0 / rNyDistKm) - (1.0 / aKm)));
    const vInfNyKmS = Math.abs(vNyCircKmS - vArrKmS);

    const rpNyKm = rNysaKm + hpNyKm;
    const eCap = 0.80; // Capture orbit
    const aCapKm = rpNyKm / (1.0 - eCap);

    const vHypNyKmS = Math.sqrt(Math.pow(vInfNyKmS, 2.0) + ((2.0 * muNysa) / rpNyKm));
    const vCapNyKmS = Math.sqrt(muNysa * ((2.0 / rpNyKm) - (1.0 / aCapKm)));
    const dvNyoiKmS = vHypNyKmS - vCapNyKmS;

    const dvTotKmS = dvTnyiMarsKmS + dvNyoiKmS;
    ...
    return {
      ...
      timeOfFlightDays: parseFloat(tofDays.toFixed(1)),
      timeOfFlightYears: parseFloat(tofYrs.toFixed(2)),
      marsDepartureDeltaVKmS: parseFloat(dvTnyiMarsKmS.toFixed(3)),
      nysaOrbitInsertionDeltaVKmS: parseFloat(dvNyoiKmS.toFixed(3)),
      totalMissionDeltaVKmS: parseFloat(dvTotKmS.toFixed(3)),
      nysaContext: `Mars-to-Nysa (${tofYrs.toFixed(2)} yr TOF, inc=${diDeg.toFixed(1)} deg, Total Delta-V=${dvTotKmS.toFixed(2)} km/s, NyOI=${dvNyoiKmS.toFixed(2)} km/s)`
    };
  }
}
