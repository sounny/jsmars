...
export class BandMathEngine {
  ...
  /**
   * Discriminate Nonahydrated Sodium-Magnesium-Ferric Hydroxyl-Sulfates (Slavkite vs Botryogen vs Voltaite) from CRISM 1.440 um, 1.940 um, 2.175 um, and 2.400 um absorption bands.
   * Reference: Bishop et al. (2009), Viviano-Beck et al. (2014), Sowe et al. (2015) for Martian Hydroxyl-Bearing Sulfates.
   * @param {number} [band1440H2ODepth=0.040] - BD1440 structural OH doublet overtone depth (0.0 to 0.40)
   * @param {number} [band1940H2ODepth=0.065] - BD1940 structural H2O fundamental depth (0.0 to 0.60)
   * @param {number} [band2175OHDepth=0.040] - BD2175 slavkite diagnostic multi-cation hydroxyl combination depth (0.0 to 0.50)
   * @param {number} [band2400SO4Depth=0.035] - BD2400 sulfate combination vibrational overtone depth (0.0 to 0.50)
   * @returns {{isMultiCationHydroxylSulfateDetected: boolean, sulfateMineralClass: string, mineralSpecies: string, chemicalFormula: string, parageneticAssociation: string}}
   */
  static computeCRISMSlavkiteSodiumMagnesiumFerricSpeciationIndices(band1440H2ODepth = 0.040, band1940H2ODepth = 0.065, band2175OHDepth = 0.040, band2400SO4Depth = 0.035) {
    const d1440 = Math.max(0.0, band1440H2ODepth);
    const d1940 = Math.max(0.0, band1940H2ODepth);
    const d2175 = Math.max(0.0, band2175OHDepth);
    const d2400 = Math.max(0.0, band2400SO4Depth);

    const isSlavkite = d1940 >= 0.045 && d1440 >= 0.025 && d2175 >= 0.025 && d2400 >= 0.025;
    const isVoltaite = d1940 >= 0.055 && d2400 >= 0.030 && d2175 < 0.025;
    const isPolyhydrate = d1940 >= 0.035 && d1440 >= 0.025 && d2175 < 0.020 && d2400 < 0.020;

    const isMultiCation = isSlavkite || isVoltaite || isPolyhydrate;
    ...
    if (isMultiCation) {
      if (isSlavkite) {
        sClass = 'Nonahydrated Slavkite Multi-Cation Hydroxyl-Sulfate Facies';
        species = 'Slavkite';
        formula = 'NaMgFe(SO4)3(OH)·9H2O';
        assoc = 'Cryogenic Evaporite Hydrothermal Slavkite Outcrop (Aram Chaos / Shalbatana / Hebes)';
      } else if (isVoltaite) {
        sClass = 'Octadecahydrated Voltaite Group Facies';
        species = 'Voltaite';
        formula = 'K2Fe2+5Fe3+3Al(SO4)12·18H2O';
        assoc = 'High-Acidity Hydrothermal Fumarolic Efflorescence';
      } else {
        sClass = 'Bloedite-Polyhydrate Sulfate Facies';
        species = 'Hydrated Sodium-Magnesium Sulfate';
        formula = 'Na2Mg(SO4)2·4H2O';
        assoc = 'Alkaline-Saline Playa Evaporite Sequence';
      }
    }

    return {
      isMultiCationHydroxylSulfateDetected: isMultiCation,
      sulfateMineralClass: sClass,
      mineralSpecies: species,
      chemicalFormula: formula,
      parageneticAssociation: assoc
  ...
  /**
   * Discriminate Hexahydrated Copper-Ferric Sulfates (Ransomite vs Chalcanthite vs Roemerite) from CRISM 0.800 um, 1.440 um, 1.940 um, and 2.400 um absorption bands.
   ...
   * @param {number} [band1940H2ODepth=0.045] - BD1940 structural H2O fundamental depth (0.0 to 0.60)
   * @param {number} [band2400SO4Depth=0.035] - BD2400 sulfate vibrational combination overtone depth (0.0 to 0.50)
   * @returns {{isCopperFerricSulfateDetected: boolean, sulfateMineralClass: string, mineralSpecies: string, chemicalFormula: string, gossanParagenesis: string}}
   */
  static computeCRISMRansomiteCopperFerricSpeciationIndices(band800CuDepth = 0.045, band1440H2ODepth = 0.035, band1940H2ODepth = 0.045, band2400SO4Depth = 0.035) {
    const d800 = Math.max(0.0, band800CuDepth);
    const d1440 = Math.max(0.0, band1440H2ODepth);
    const d1940 = Math.max(0.0, band1940H2ODepth);
    const d2400 = Math.max(0.0, band2400SO4Depth);

    const isRansomite = d800 >= 0.030 && d1440 >= 0.020 && d1940 >= 0.030 && d2400 >= 0.025;
    const isChalcanthite = d800 >= 0.035 && d1940 >= 0.040 && d2400 < 0.020;
    const isRoemerite = d1940 >= 0.035 && d2400 >= 0.025 && d800 < 0.020;

    const isCuFe = isRansomite || isChalcanthite || isRoemerite;

    let sClass = 'Copper-Ferric-Sulfate-Free Silicate Matrix';
    let species = 'Basaltic Matrix';
    let formula = 'Silicate Matrix';
    let gossan = 'Unaltered Primary Crust';

    if (isCuFe) {
      if (isRansomite) {
        sClass = 'Hexahydrated Ransomite Copper-Ferric Sulfate Facies';
        species = 'Ransomite';
        formula = 'CuFe2(SO4)4·6H2O';
        gossan = 'Oxidized Copper-Ferric Sulfide Gossan Outcrop (Nili Fossae / Terra Sabaea / Noctis)';
      } else if (isChalcanthite) {
        sClass = 'Pentahydrated Chalcanthite Copper Sulfate Facies';
        species = 'Chalcanthite';
        formula = 'CuSO4·5H2O';
        gossan = 'Supergene Copper Sulfate Efflorescence';
      } else {
        sClass = 'Tetradecahydrated Roemerite Ferrous-Ferric Sulfate Facies';
        species = 'Roemerite';
        formula = 'Fe2+Fe3+2(SO4)4·14H2O';
        gossan = 'Acid Mine Drainage / Hydrothermal Iron-Sulfate Matrix';
      }
    }

    return {
      isCopperFerricSulfateDetected: isCuFe,
      sulfateMineralClass: sClass,
      mineralSpecies: species,
      chemicalFormula: formula,
  ...
  /**
   * Discriminate Octahydrated Ferric Oxysulfates (Castanite vs Amarantite vs Copiapite) from CRISM 1.440 um, 1.940 um, 2.160 um, and 2.400 um absorption bands.
   * Reference: Bishop et al. (2009), Viviano-Beck et al. (2014), Sowe et al. (2015) for Martian Iron-Oxysulfates.
   * @param {number} [band1440H2ODepth=0.040] - BD1440 structural OH doublet overtone depth (0.0 to 0.40)
   * @param {number} [band1940H2ODepth=0.060] - BD1940 structural H2O fundamental depth (0.0 to 0.60)
   * @param {number} [band2160FeOHDepth=0.040] - BD2160 castanite diagnostic ferric oxysulfate vibrational combination depth (0.0 to 0.50)
   * @param {number} [band2400SO4Depth=0.035] - BD2400 sulfate vibrational combination overtone depth (0.0 to 0.50)
   * @returns {{isFerricOxysulfateDetected: boolean, sulfateMineralClass: string, mineralSpecies: string, chemicalFormula: string, parageneticAssociation: string}}
   */
  static computeCRISMCastaniteFerricOxysulfateSpeciationIndices(band1440H2ODepth = 0.040, band1940H2ODepth = 0.060, band2160FeOHDepth = 0.040, band2400SO4Depth = 0.035) {
    const d1440 = Math.max(0.0, band1440H2ODepth);
    const d1940 = Math.max(0.0, band1940H2ODepth);
    const d2160 = Math.max(0.0, band2160FeOHDepth);
    const d2400 = Math.max(0.0, band2400SO4Depth);

    const isCastanite = d1940 >= 0.050 && d1440 >= 0.025 && d2160 >= 0.025 && d2400 >= 0.025;
    const isAmarantite = d1940 >= 0.030 && d1940 < 0.050 && d2160 >= 0.025 && d2400 >= 0.025;
    const isCopiapite = d1940 >= 0.055 && d2400 >= 0.030 && d2160 < 0.020;

    const isOxysulfate = isCastanite || isAmarantite || isCopiapite;

    let sClass = 'Ferric-Oxysulfate-Free Silicate Matrix';
    let species = 'Basaltic Matrix';
    let formula = 'Silicate Matrix';
    let assoc = 'Unaltered Primary Crust';
    ...
    if (isOxysulfate) {
      if (isCastanite) {
        sClass = 'Octahydrated Castanite Ferric Oxysulfate Facies';
        species = 'Castanite';
        formula = 'Fe2(SO4)2O·8H2O';
        assoc = 'Acidic Evaporite Castanite Layer (Aureum / Juventae / Ganges)';
      } else if (isAmarantite) {
        sClass = 'Heptahydrated Amarantite Oxysulfate Facies';
        species = 'Amarantite';
        formula = 'Fe2(SO4)2O·7H2O';
        assoc = 'Partially Dehydrated Secondary Oxysulfate Outcrop';
      } else {
        sClass = 'Copiapite-Group Ferric Sulfate Facies';
        species = 'Copiapite';
        formula = 'Fe2+Fe3+4(SO4)6(OH)2·20H2O';
        assoc = 'Hyper-Acidic Aqueous Efflorescence Sequence';
      }
    }

    return {
      isFerricOxysulfateDetected: isOxysulfate,
      sulfateMineralClass: sClass,
      mineralSpecies: species,
      chemicalFormula: formula,
      parageneticAssociation: assoc
  ...
  /**
   * Discriminate Anhydrous Copper-Ferric Sulfates (Anhydrous Cu-Fe Sulfate vs Ransomite vs Chalcanthite) from CRISM 0.800 um, 1.440 um, 1.940 um, and 2.400 um absorption bands.
   * Reference: Bishop et al. (2009), Viviano-Beck et al. (2014), Sowe et al. (2015) for Martian Copper-Bearing Sulfates.
   * @param {number} [band800CuDepth=0.045] - BD800 copper crystal field electronic absorption depth (0.0 to 0.50)
   * @param {number} [band1440H2ODepth=0.010] - BD1440 structural OH doublet overtone depth (0.0 to 0.40)
   * @param {number} [band1940H2ODepth=0.015] - BD1940 structural H2O fundamental depth (0.0 to 0.60)
   * @param {number} [band2400SO4Depth=0.045] - BD2400 sulfate vibrational combination overtone depth (0.0 to 0.50)
   * @returns {{isAnhydrousCopperFerricSulfateDetected: boolean, sulfateMineralClass: string, mineralSpecies: string, chemicalFormula: string, gossanHydrationState: string}}
   */
  static computeCRISMAnhydrousCopperFerricSulfateSpeciationIndices(band800CuDepth = 0.045, band1440H2ODepth = 0.010, band1940H2ODepth = 0.015, band2400SO4Depth = 0.045) {
    const d800 = Math.max(0.0, band800CuDepth);
    const d1440 = Math.max(0.0, band1440H2ODepth);
    const d1940 = Math.max(0.0, band1940H2ODepth);
    const d2400 = Math.max(0.0, band2400SO4Depth);

    const isAnhydrousCuFe = d800 >= 0.030 && d2400 >= 0.030 && d1940 < 0.020;
    const isRansomite = d800 >= 0.030 && d1440 >= 0.020 && d1940 >= 0.030 && d2400 >= 0.025;
    const isChalcanthite = d800 >= 0.035 && d1940 >= 0.040 && d2400 < 0.020;

    const isCuFeSulfate = isAnhydrousCuFe || isRansomite || isChalcanthite;
    ...
    if (isCuFeSulfate) {
      if (isAnhydrousCuFe) {
        sClass = 'Anhydrous Copper-Ferric Sulfate Gossan Facies';
        species = 'Anhydrous Copper-Ferric Sulfate';
        formula = 'CuFe2(SO4)4';
        state = 'Desiccated High-Temperature Sulfide Gossan Outcrop (Nili Fossae / Terra Sabaea / Noctis)';
      } else if (isRansomite) {
        sClass = 'Hexahydrated Ransomite Copper-Ferric Sulfate Facies';
        species = 'Ransomite';
        formula = 'CuFe2(SO4)4·6H2O';
        state = 'Hydrated Supergene Sulfide Gossan Outcrop';
      } else {
        sClass = 'Pentahydrated Chalcanthite Copper Sulfate Facies';
        species = 'Chalcanthite';
        formula = 'CuSO4·5H2O';
        state = 'Supergene Copper Sulfate Efflorescence';
      }
    }

    return {
      isAnhydrousCopperFerricSulfateDetected: isCuFeSulfate,
      sulfateMineralClass: sClass,
      mineralSpecies: species,
      chemicalFormula: formula,
  ...
  /**
   * Discriminate Anhydrous Multi-Cation Sulfates (Anhydrous Slavkite vs Slavkite Nonahydrate vs Bloedite) from CRISM 1.440 um, 1.940 um, 2.175 um, and 2.400 um absorption bands.
   * Reference: Bishop et al. (2009), Viviano-Beck et al. (2014), Sowe et al. (2015) for Martian Multi-Cation Sulfates.
   * @param {number} [band1440H2ODepth=0.010] - BD1440 structural OH doublet overtone depth (0.0 to 0.40)
   * @param {number} [band1940H2ODepth=0.015] - BD1940 structural H2O fundamental depth (0.0 to 0.60)
   * @param {number} [band2175OHDepth=0.040] - BD2175 slavkite diagnostic cation vibrational combination depth (0.0 to 0.50)
   * @param {number} [band2400SO4Depth=0.045] - BD2400 sulfate vibrational combination overtone depth (0.0 to 0.50)
   * @returns {{isAnhydrousMultiCationSulfateDetected: boolean, sulfateMineralClass: string, mineralSpecies: string, chemicalFormula: string, evaporiteEnvironment: string}}
   */
  static computeCRISMAnhydrousSlavkiteSpeciationIndices(band1440H2ODepth = 0.010, band1940H2ODepth = 0.015, band2175OHDepth = 0.040, band2400SO4Depth = 0.045) {
    const d1440 = Math.max(0.0, band1440H2ODepth);
    const d1940 = Math.max(0.0, band1940H2ODepth);
    const d2175 = Math.max(0.0, band2175OHDepth);
    const d2400 = Math.max(0.0, band2400SO4Depth);

    const isAnhydrousSlv = d2175 >= 0.025 && d2400 >= 0.030 && d1940 < 0.020;
    const isSlavkite = d2175 >= 0.025 && d1440 >= 0.020 && d1940 >= 0.030 && d2400 >= 0.025;
    const isBloedite = d1940 >= 0.035 && d2400 >= 0.030 && d2175 < 0.020;

    const isMultiCation = isAnhydrousSlv || isSlavkite || isBloedite;

    let sClass = 'Multi-Cation-Sulfate-Free Silicate Matrix';
    let species = 'Basaltic Matrix';
    let formula = 'Silicate Matrix';
    let env = 'Unaltered Primary Crust';
    ...
    if (isMultiCation) {
      if (isAnhydrousSlv) {
        sClass = 'Anhydrous Multi-Cation Sulfate Facies';
        species = 'Anhydrous Slavkite';
        formula = 'NaMgFe(SO4)3O';
        env = 'Desiccated High-Temperature Multi-Cation Evaporite Crust (Aram / Shalbatana / Hebes)';
      } else if (isSlavkite) {
        sClass = 'Nonahydrated Slavkite Multi-Cation Sulfate Facies';
        species = 'Slavkite';
        formula = 'NaMgFe(SO4)3(OH)·9H2O';
        env = 'Cryogenic Hydrothermal Multi-Cation Evaporite Outcrop';
      } else {
        sClass = 'Tetrahydrated Bloedite Sodium-Magnesium Sulfate Facies';
        species = 'Bloedite';
        formula = 'Na2Mg(SO4)2·4H2O';
        env = 'Alkaline Saline Playa Evaporite Sequence';
      }
    }

    return {
      isAnhydrousMultiCationSulfateDetected: isMultiCation,
      sulfateMineralClass: sClass,
      mineralSpecies: species,
      chemicalFormula: formula,
