...
export class KRCEngine {
  ...
  /**
   * Calculate low-temperature suboxic hydrothermal metasomatism of vesicular basalt into green celadonite mica, amygdaloidal infilling, and thermal inertia.
   * Basalt Vesicles + K+ + Fe3+ + (Mg,Fe2+) + SiO2(aq) + H2O (30-120 C) -> Celadonite (K(Mg,Fe2+)Fe3+Si4O10(OH)2)
   * Reference: Ehlmann et al. (2011), Viviano-Beck et al. (2014), Michalski et al. (2015) for Martian Green Mica Terranes.
   * @param {number} [initialBasaltVesiclePorosity=0.20] - Initial vesicular basalt porosity (0.02 to 0.45)
   * @param {number} [hydrothermalTempC=60.0] - Low-temperature hydrothermal fluid temperature in C (15 to 160 C)
   * @param {number} [potassiumIronActivityProduct=0.25] - Dissolved K-Fe fluid activity product (0.01 to 0.80)
   * @param {number} [durationYears=350.0] - Metasomatic exposure duration in years (0.1 to 5000 yr)
   * @returns {{celadoniteConversionFraction: number, boundHydroxylYieldWeightPercent: number, dominantMicaSpecies: string, amygdaloidalBasaltThermalInertiaTIU: number, micaFaciesClass: string, celadoniteContext: string}}
   */
  static computeMartianCeladoniteMetasomatism(initialBasaltVesiclePorosity = 0.20, hydrothermalTempC = 60.0, potassiumIronActivityProduct = 0.25, durationYears = 350.0) {
    const phi0 = Math.max(0.01, Math.min(0.50, initialBasaltVesiclePorosity));
    const TC = Math.max(10.0, Math.min(200.0, hydrothermalTempC));
    const aKFe = Math.max(0.005, Math.min(1.0, potassiumIronActivityProduct));
    const tYrs = Math.max(0.01, durationYears);

    const TK = TC + 273.15;
    const tSec = tYrs * 365.25 * 86400.0;
    const Rgas = 8.314;
    const Ea = 4.30e4; // 43 kJ/mol for celadonite crystallization

    // Reaction rate constant
    const kRate = 4.8e-3 * Math.exp(-Ea / (Rgas * TK)) * Math.pow(aKFe, 0.35);
    const alphaCel = 1.0 - Math.exp(-Math.min(20.0, kRate * tSec));

    // Bound hydroxyl yield (wt%)
    const wOHPct = alphaCel * 4.65;

    // Amygdaloidal vesicle infilling and crystalline densification
    const phiResidual = phi0 * (1.0 - (0.75 * alphaCel));
    const rhoGrain = 2950.0;
    const rhoBulk = ((1.0 - phiResidual) * rhoGrain) + (phiResidual * 1000.0);
    ...
    let species = 'Green Celadonite Mica (K(Mg,Fe2+)Fe3+Si4O10(OH)2)';
    let mClass = 'Ferric-Ferrous Green Mica Facies';

    if (alphaCel >= 0.50 && TC >= 30.0 && TC <= 120.0 && aKFe >= 0.12) {
      species = 'Celadonite (K(Mg,Fe2+)Fe3+Si4O10(OH)2)';
      mClass = 'Suboxic Celadonite Green Mica Facies (Nili Fossae / Mawrth / Noctis)';
    } else if (TC > 120.0) {
      species = 'Glauconite-Illite Transition Sequence';
      mClass = 'Moderate-Temperature Illite-Mica Facies';
    } else {
      species = 'Nontronite-Celadonite Mixed-Layer Precursor';
      mClass = 'Incipient Smectite-Mica Alteration Crust';
    }
    ...
    return {
      ...
      dominantMicaSpecies: species,
      amygdaloidalBasaltThermalInertiaTIU: parseFloat(TIU.toFixed(1)),
      micaFaciesClass: mClass,
  ...
  /**
   * Calculate low-temperature acidic oxidative hydration of jarosite and ferric-ferrous sulfate crust into polyhydrated copiapite, superhydration swelling, and thermal inertia.
   * Jarosite + Fe2+ + 2 Fe3+ + 4 SO4(2-) + 22 H2O (-5 to 35 C) -> Copiapite (Fe2+Fe3+4(SO4)6(OH)2·20H2O)
   * Reference: Bishop et al. (2009), Viviano-Beck et al. (2014), Sowe et al. (2015) for Martian Polyhydrated Ferric Sulfates.
   * @param {number} [initialJarositicCrustPorosity=0.32] - Initial jarositic host crust porosity (0.05 to 0.60)
   * @param {number} [ambientTempC=15.0] - Ambient alteration temperature in C (-15 to 55 C)
   * @param {number} [acidFerricSulfateFluidActivity=0.36] - Dissolved acid ferric sulfate fluid activity (0.01 to 1.0)
   * @param {number} [durationYears=200.0] - Metasomatic hydration duration in years (0.1 to 5000 yr)
   * @returns {{copiapiteConversionFraction: number, boundWaterYieldWeightPercent: number, dominantFerricSulfateSpecies: string, friableSulfateThermalInertiaTIU: number, ferricSulfateFaciesClass: string, copiapiteContext: string}}
   */
  static computeMartianCopiapiteMetasomatism(initialJarositicCrustPorosity = 0.32, ambientTempC = 15.0, acidFerricSulfateFluidActivity = 0.36, durationYears = 200.0) {
    const phi0 = Math.max(0.01, Math.min(0.65, initialJarositicCrustPorosity));
    const TC = Math.max(-20.0, Math.min(70.0, ambientTempC));
    const aFeSO4 = Math.max(0.005, Math.min(1.0, acidFerricSulfateFluidActivity));
    const tYrs = Math.max(0.01, durationYears);

    const TK = TC + 273.15;
    const tSec = tYrs * 365.25 * 86400.0;
    const Rgas = 8.314;
    const Ea = 3.80e4; // 38 kJ/mol for copiapite crystallization

    // Reaction rate constant
    const kRate = 5.1e-3 * Math.exp(-Ea / (Rgas * TK)) * Math.pow(aFeSO4, 0.35);
    const alphaCop = 1.0 - Math.exp(-Math.min(20.0, kRate * tSec));

    // Bound crystal water yield (wt%)
    const wH2OPct = alphaCop * 29.80;

    // Superhydration crystal expansion and friable porous texture
    const phiResidual = (phi0 * (1.0 - (0.30 * alphaCop))) + (0.10 * alphaCop);
    const rhoGrain = 2100.0;
    const rhoBulk = ((1.0 - phiResidual) * rhoGrain) + (phiResidual * 1000.0);
    ...
    let species = 'Polyhydrated Copiapite (Fe2+Fe3+4(SO4)6(OH)2·20H2O)';
    let cClass = 'Polyhydrated Mixed-Valence Ferric Sulfate Facies';

    if (alphaCop >= 0.50 && TC >= -5.0 && TC <= 35.0 && aFeSO4 >= 0.15) {
      species = 'Copiapite (Fe2+Fe3+4(SO4)6(OH)2·20H2O)';
      cClass = 'Acidic Evaporite Copiapite Facies (Juventae / Capri / Melas Chasma)';
    } else if (TC > 35.0) {
      species = 'Coquimbite Nonahydrate (Fe2(SO4)3·9H2O)';
      cClass = 'Moderate-Temperature Ferric Sulfate Facies';
    } else {
      species = 'Rhomboclase Acid Sulfate Precursor';
      cClass = 'Hyper-Acidic Ferric Sulfate Residue';
    }

    return {
      copiapiteConversionFraction: parseFloat(alphaCop.toFixed(3)),
      boundWaterYieldWeightPercent: parseFloat(wH2OPct.toFixed(2)),
      dominantFerricSulfateSpecies: species,
      friableSulfateThermalInertiaTIU: parseFloat(TIU.toFixed(1)),
      ferricSulfateFaciesClass: cClass,
  ...
  /**
   * Calculate extreme thermal desiccation and anhydrous transition of tetrahydrated hohmannite into sesquihydrated metahohmannite, crystal sintering, and thermal inertia.
   * Fe2(SO4)2O·4H2O -> Fe2(SO4)2O·1.5H2O + 2.5 H2O (40 to 120 C, hyper-arid low RH)
   * Reference: Bishop et al. (2009), Viviano-Beck et al. (2014), Sowe et al. (2015) for Martian Iron-Oxysulfates.
   * @param {number} [initialHohmannitePorosity=0.25] - Initial hohmannite evaporite porosity (0.05 to 0.60)
   * @param {number} [surfaceTempC=55.0] - Surface/hydrothermal desiccation temperature in C (20 to 130 C)
   * @param {number} [atmosphericRelativeHumidity=0.05] - Atmospheric / pore relative humidity (0.001 to 0.70)
   * @param {number} [durationYears=280.0] - Desiccation exposure duration in years (0.1 to 5000 yr)
   * @returns {{metahohmanniteConversionFraction: number, boundWaterYieldWeightPercent: number, dominantFerricOxysulfatePhase: string, induratedSesquihydrateThermalInertiaTIU: number, sesquihydrateFaciesClass: string, metahohmanniteContext: string}}
   */
  static computeMartianMetahohmanniteDehydration(initialHohmannitePorosity = 0.25, surfaceTempC = 55.0, atmosphericRelativeHumidity = 0.05, durationYears = 280.0) {
    const phi0 = Math.max(0.01, Math.min(0.65, initialHohmannitePorosity));
    const TC = Math.max(0.0, Math.min(145.0, surfaceTempC));
    ...
    const tSec = tYrs * 365.25 * 86400.0;
    const Rgas = 8.314;
    const Ea = 4.40e4; // 44.0 kJ/mol for metahohmannite dehydration crystallization

    // Reaction rate constant
    const kRate = 3.8e-3 * Math.exp(-Ea / (Rgas * TK)) * Math.pow(1.0 - rh, 0.60);
    const alphaMet = 1.0 - Math.exp(-Math.min(20.0, kRate * tSec));

    // Bound water yield (wt%)
    const wH2OPct = 8.42 + ((1.0 - alphaMet) * 10.95);

    // Crystal volume contraction and indurated sesquihydrate compaction
    const phiResidual = phi0 * (1.0 - (0.45 * alphaMet));
    const rhoGrain = 2780.0;
    const rhoBulk = ((1.0 - phiResidual) * rhoGrain) + (phiResidual * 1000.0);
    ...
    let species = 'Metahohmannite (Fe2(SO4)2O·1.5H2O)';
    let mClass = 'Sesquihydrated Ferric Oxysulfate Facies';

    if (alphaMet >= 0.50 && TC >= 40.0 && rh <= 0.20) {
      species = 'Metahohmannite (Fe2(SO4)2O·1.5H2O)';
      mClass = 'Indurated Metahohmannite Facies (Coprates / Ganges / Valles Marineris)';
    } else if (rh > 0.20) {
      species = 'Metastable Hohmannite-Metahohmannite Assemblage';
      mClass = 'Partially Dehydrated Transition Crust';
    } else {
      species = 'Anhydrous Iron Sulfate / Hematite Sintered Matrix';
      mClass = 'Hyper-Thermal Calcination Crust';
    }

    return {
      metahohmanniteConversionFraction: parseFloat(alphaMet.toFixed(3)),
      boundWaterYieldWeightPercent: parseFloat(wH2OPct.toFixed(2)),
      dominantFerricOxysulfatePhase: species,
      induratedSesquihydrateThermalInertiaTIU: parseFloat(TIU.toFixed(1)),
      sesquihydrateFaciesClass: mClass,
      metahohmanniteContext: `Metahohmannite at ${TC.toFixed(0)} C, RH=${(rh * 100).toFixed(0)}% (${(alphaMet * 100).toFixed(1)}% dehydrated, ${wH2OPct.toFixed(2)}% bound H2O, TIU=${TIU.toFixed(0)}, ${mClass})`
    };
  }

  /**
   * Calculate hydrothermal desiccation and high-temperature dehydration of hexahydrated ransomite into anhydrous copper-ferric sulfate gossan, crystal contraction, and thermal inertia.
   * CuFe2(SO4)4·6H2O -> CuFe2(SO4)4 + 6 H2O (35 to 110 C, low RH)
   * Reference: Bishop et al. (2009), Viviano-Beck et al. (2014), Sowe et al. (2015) for Martian Copper-Bearing Sulfates.
   * @param {number} [initialRansomitePorosity=0.22] - Initial ransomite evaporite porosity (0.05 to 0.55)
   * @param {number} [surfaceTempC=48.0] - Surface/hydrothermal desiccation temperature in C (15 to 120 C)
   * @param {number} [atmosphericRelativeHumidity=0.08] - Atmospheric / pore relative humidity (0.001 to 0.70)
   * @param {number} [durationYears=250.0] - Desiccation exposure duration in years (0.1 to 5000 yr)
   * @returns {{ransomiteDehydrationFraction: number, boundWaterYieldWeightPercent: number, dominantCopperFerricPhase: string, induratedAnhydrousThermalInertiaTIU: number, anhydrousSulfateFaciesClass: string, ransomiteDehydrationContext: string}}
   */
  static computeMartianRansomiteDehydration(initialRansomitePorosity = 0.22, surfaceTempC = 48.0, atmosphericRelativeHumidity = 0.08, durationYears = 250.0) {
    const phi0 = Math.max(0.01, Math.min(0.60, initialRansomitePorosity));
    ...
    const tSec = tYrs * 365.25 * 86400.0;
    const Rgas = 8.314;
    const Ea = 4.35e4; // 43.5 kJ/mol for ransomite dehydration crystallization

    // Reaction rate constant
    const kRate = 4.0e-3 * Math.exp(-Ea / (Rgas * TK)) * Math.pow(1.0 - rh, 0.50);
    const alphaRdh = 1.0 - Math.exp(-Math.min(20.0, kRate * tSec));

    // Bound water yield (wt%)
    const wH2OPct = (1.0 - alphaRdh) * 16.89;

    // Crystal volume contraction and indurated anhydrous compaction
    const phiResidual = phi0 * (1.0 - (0.45 * alphaRdh));
    const rhoGrain = 2950.0;
    const rhoBulk = ((1.0 - phiResidual) * rhoGrain) + (phiResidual * 1000.0);
    ...
    let species = 'Anhydrous Copper-Ferric Sulfate (CuFe2(SO4)4)';
    let aClass = 'Anhydrous Copper-Ferric Sulfate Gossan Facies';

    if (alphaRdh >= 0.50 && TC >= 35.0 && rh <= 0.20) {
      species = 'Anhydrous Copper-Ferric Sulfate (CuFe2(SO4)4)';
      aClass = 'Indurated Anhydrous Cu-Fe Gossan Facies (Nili Fossae / Terra Sabaea / Noctis)';
    } else if (rh > 0.20) {
      species = 'Metastable Ransomite-Anhydrous Mixture';
      aClass = 'Partially Dehydrated Transition Phase';
    } else {
      species = 'Chalcocyanite-Yavapaiite Calcination Matrix';
      aClass = 'Hyper-Thermal Calcination Residue';
    }

    return {
      ransomiteDehydrationFraction: parseFloat(alphaRdh.toFixed(3)),
      boundWaterYieldWeightPercent: parseFloat(wH2OPct.toFixed(2)),
      dominantCopperFerricPhase: species,
      induratedAnhydrousThermalInertiaTIU: parseFloat(TIU.toFixed(1)),
      anhydrousSulfateFaciesClass: aClass,
      ransomiteDehydrationContext: `Anhydrous Cu-Fe at ${TC.toFixed(0)} C, RH=${(rh * 100).toFixed(0)}% (${(alphaRdh * 100).toFixed(1)}% dehydrated, ${wH2OPct.toFixed(2)}% bound H2O, TIU=${TIU.toFixed(0)}, ${aClass})`
    };
  }

  /**
   * Calculate hydrothermal desiccation and high-temperature dehydration of tetrahydrated guildite into anhydrous copper-ferric oxysulfate, crystal sintering, and thermal inertia.
   * CuFe(SO4)2(OH)·4H2O -> CuFe(SO4)2O + 4.5 H2O (35 to 115 C, low RH)
   * Reference: Bishop et al. (2009), Viviano-Beck et al. (2014), Sowe et al. (2015) for Martian Copper-Bearing Sulfates.
   * @param {number} [initialGuilditePorosity=0.24] - Initial guildite host rock porosity (0.05 to 0.55)
   * @param {number} [surfaceTempC=50.0] - Surface/hydrothermal desiccation temperature in C (15 to 125 C)
   * @param {number} [atmosphericRelativeHumidity=0.06] - Atmospheric / pore relative humidity (0.001 to 0.70)
   * @param {number} [durationYears=260.0] - Desiccation exposure duration in years (0.1 to 5000 yr)
   * @returns {{guilditeDehydrationFraction: number, boundWaterYieldWeightPercent: number, dominantCopperFerricPhase: string, induratedAnhydrousThermalInertiaTIU: number, anhydrousOxysulfateFaciesClass: string, guilditeDehydrationContext: string}}
   */
  static computeMartianGuilditeDehydration(initialGuilditePorosity = 0.24, surfaceTempC = 50.0, atmosphericRelativeHumidity = 0.06, durationYears = 260.0) {
    const phi0 = Math.max(0.01, Math.min(0.60, initialGuilditePorosity));
    ...
    const tSec = tYrs * 365.25 * 86400.0;
    const Rgas = 8.314;
    const Ea = 4.30e4; // 43.0 kJ/mol for guildite dehydration crystallization

    // Reaction rate constant
    const kRate = 4.1e-3 * Math.exp(-Ea / (Rgas * TK)) * Math.pow(1.0 - rh, 0.55);
    const alphaGdh = 1.0 - Math.exp(-Math.min(20.0, kRate * tSec));

    // Bound water and OH yield (wt%)
    const wH2OPct = (1.0 - alphaGdh) * 24.35;

    // Crystal volume contraction and indurated anhydrous compaction
    const phiResidual = phi0 * (1.0 - (0.42 * alphaGdh));
    const rhoGrain = 2820.0;
    const rhoBulk = ((1.0 - phiResidual) * rhoGrain) + (phiResidual * 1000.0);

    const kTherm = 1.80; // W/(m K)
    const Cspec = 840.0; // J/(kg K)
    const TIU = Math.sqrt(kTherm * rhoBulk * Cspec);

    let species = 'Anhydrous Copper-Ferric Oxysulfate (CuFe(SO4)2O)';
    let gClass = 'Anhydrous Copper-Ferric Oxysulfate Facies';

    if (alphaGdh >= 0.50 && TC >= 35.0 && rh <= 0.20) {
      species = 'Anhydrous Copper-Ferric Oxysulfate (CuFe(SO4)2O)';
      gClass = 'Indurated Anhydrous Cu-Fe Oxysulfate Facies (Syrtis Major / Coprates / Noctis)';
    } else if (rh > 0.20) {
      species = 'Metastable Guildite-Anhydrous Composite';
      gClass = 'Partially Dehydrated Hydroxyl-Sulfate Phase';
    } else {
      species = 'Tenorite-Hematite Sintered Calcination Matrix';
      gClass = 'Hyper-Thermal Calcination Crust';
    }

    return {
      guilditeDehydrationFraction: parseFloat(alphaGdh.toFixed(3)),
      boundWaterYieldWeightPercent: parseFloat(wH2OPct.toFixed(2)),
      dominantCopperFerricPhase: species,
      induratedAnhydrousThermalInertiaTIU: parseFloat(TIU.toFixed(1)),
      anhydrousOxysulfateFaciesClass: gClass,
